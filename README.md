...
- 可复算的大正藏 T01–T37 二十一个固定子集、1,844 条汉译来源逐文件清单；
- 可被下一代维护者接管的开放代码与文档。
...
生产环境设置 `CORPUS_ASSET_BASE_URL=https://canon.foxue.ai` 后读取边缘语料。变量未设置、R2 尚未启用或边缘读取失败时，服务端自动回退到仓库内已核验的 2,076 个完整全文记录和 43 个节译、后分、节本、短本、残篇、组件、局部、分离、版本或已知缺文见证，因此经典阅读不依赖单一供应商存活。CBETA 大正藏 T01–T37 二十一个固定来源子集均已逐文件闭合并保留 1,844 个 TEI 来源资产；T33–T37 的根本经、整经注疏、章节注释、游意、纲目、讲说记录、治定本、合注、再注释、略策、颂释、论、卷意、观门、集解与同题异作分层建模，不把注疏冒充佛陀亲说。固定 SuttaCentral 提交的 7,288 份巴利 root 已全部受控：经藏 5,764 份与 284,702 个原生稳定段落，律藏 422 份、6 个书级表达与 71,557 个可读稳定段落，论藏 1,102 份、七论书级表达与 88,414 个稳定段落；另有 24 份梵文与俗语 root 按 3 个表达保存。经、律、论、疏分别统计，物理文件、经号、文本表达、版本见证、暂定书目实体、书级集合和规范作品始终分层；固定来源内 100% 绝不冒充全球佛经作品覆盖率。
