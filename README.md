...
- 从问题回到原典的引证式问经体验；
- 本地优先的佛典研究任务：声明问题与来源范围，把选文整理为主张—证据矩阵并导出开放 Markdown；
- 带稳定段落标识和来源说明的经典阅读；
...
截至 2026-08-28，Cloudflare 自定义域名 [`canon.foxue.ai`](https://canon.foxue.ai/health) 已由 R2 只读 Worker 提供服务：`/health`、`/ready` 与 `/v1/latest.json` 均从公网返回 200。当前指针是 `gbcr-6.27.0-2b8ab8d5e4fe-eac6c24781dd-a582cf471b7c-f7eed38c2e74`，清单 SHA-256 为 `82cd1aeda32a5593417cac81fd3a595eceb288aa13a10b35f4e6fb7e0dbdc843`；清单固定 284,525 个不可变内容对象、2,893,423,673 字节、4,190 个文本表达、265,253 个阅读版页与 5,945,851 个稳定行段。Worker 只允许白名单内的读取方法。这个三态协议仍将“边缘入口存活”“发行指针可发现”“完整不可变对象已就绪”分开，防止监控与维护者把半完成发布误判为完整保存。`.github/workflows/cloudflare-edge-health.yml` 每日从公网核对 Cloudflare 权威 DNS、Worker 身份、安全头、发行 ID、清单哈希、CORS、只读门禁、Müller《法句经》、Gemmell《金刚经》与 Soothill《法华经》英译节本代表对象及就绪语义。

生产环境设置 `CORPUS_ASSET_BASE_URL=https://canon.foxue.ai` 后读取边缘语料。变量未设置、R2 尚未启用或边缘读取失败时，服务端自动回退到仓库内已核验的 4,144 个完整来源表达或版本见证和 46 个节译、后分、节本、短本、残篇、组件、局部、分离、版本或已知缺文来源记录，因此经典阅读不依赖单一供应商存活。当前登记册包含 3,396 个可追踪作品实体或权威候选，其中 3,369 个作品至少有一个完整来源表达。CBETA 大正藏 T01–T55 与 T85 四十个固定来源子集均已逐文件闭合并保留 2,471 个 TEI 来源资产；固定 SuttaCentral 提交中的巴利、印度语与古汉译 root、Esukhia 德格《甘珠尔》001–102 卷，以及经权利核验的现代日英译本继续按 Work、Expression 与 Witness 分层保存。v6.26 新增 William Gemmell 1912 年《金刚经》英译，保留原书合并的第三、四章；v6.27 新增 W. E. Soothill 1930 年据鸠摩罗什汉译所出的《法华经》英译见证，复用既有 Saddharmapuṇḍarīka／T0262 作品并保存 28 品与 511 个稳定段落。Soothill 来源的 28 品记录完整，但译者明示省略重复与部分细节，因此页面标为“节译见证”，不得称作无删节全文。两项来源均只依据 Project Gutenberg 的美国公版声明，不据此推断全球公版。当前独立真人决定和自动全球分母变更均为 0；全球作品、全文、中文翻译、权利和质量覆盖率继续保持 `null`，固定来源内的完整性绝不冒充全球佛经作品覆盖率或佛陀逐字亲说比例。
