...
生产环境设置 `CORPUS_ASSET_BASE_URL=https://canon.foxue.ai` 后读取边缘语料。变量未设置、R2 尚未启用或边缘读取失败时，服务端自动回退到仓库内已核验的 1,136 个完整全文记录和 14 个节译、后分、节本、短本或残篇见证，因此经典阅读不依赖单一供应商存活。CBETA 大正藏 T01–T17 固定来源均已逐文件闭合并保留 882 个 TEI 来源资产。巴利经藏保留 5,764 个 Bilara root 与 284,702 个原生稳定段落，巴利律藏保留 422 个 root、6 个书级表达与 71,557 个可读稳定段落；另有 24 份梵文与俗语 root 按 3 个表达保存。经、律、论分别统计，物理文件、经号、文本表达、版本见证、暂定书目实体、书级集合和规范作品始终分层；这些固定来源完整性绝不冒充全球佛经作品覆盖率。
