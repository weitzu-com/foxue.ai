...
截至 2026-08-25，Cloudflare 自定义域名 [`canon.foxue.ai`](https://canon.foxue.ai/health) 已由 R2 只读 Worker 提供服务：`/health`、`/ready` 与 `/v1/latest.json` 均从公网返回 200。当前指针是 `gbcr-6.22.0-2b8ab8d5e4fe-eac6c24781dd-a582cf471b7c-cd4264170b73`，清单 SHA-256 为 `17754f9d63eedbc9c930d77f86dfe355316e79bbd3a44fbadc9607e749c05391`；Worker 只允许白名单内的读取方法。这个三态协议仍将“边缘入口存活”“发行指针可发现”“完整不可变对象已就绪”分开，防止监控与维护者把半完成发布误判为完整保存。`.github/workflows/cloudflare-edge-health.yml` 每日从公网核对 Cloudflare 权威 DNS、Worker 身份、安全头、发行 ID、清单哈希、CORS、只读门禁与就绪语义。
