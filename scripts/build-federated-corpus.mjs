...
const inputs = {
  ...
  snapshots: "data/gbcr/source-snapshots-v2.5.0.json",
  ...
  cbetaBatch: "data/corpus/cbeta/batch-v4.4.0.json",
  cbetaCatalog: "data/corpus/cbeta/catalog-v4.4.0.json",
  cbetaManifest: "data/corpus/cbeta/manifest-v4.4.0.json",
  cbetaRegistry: "data/gbcr/registry-cbeta-v4.4.0.json",
...
const outputPath = resolve(root, "data/gbcr/registry-v5.5.0.json");
...
if (
  cbetaBatch.version !== "4.4.0" || cbetaBatch.files.length !== 21 ||
  ...
  cbetaBatch.collection.newSourceBytes !== 10041102 ||
  cbetaBatch.collection.newStableSegments !== 77568 ||
  cbetaBatch.collection.newFolios !== 2774 ||
  cbetaBatch.collection.newJuans !== 117 ||
  ...
  cbetaCatalog.files.length !== 1830 || cbetaManifest.files.length !== 1830 ||
  cbetaRegistry.registry.version !== "4.4.0" || cbetaRegistry.works.length !== 1612 ||
  cbetaRegistry.works.flatMap((work) => work.expressions).length !== 1830
...
if (
  cbetaFamily?.controlledExpressionRecords !== 1844 ||
  cbetaFamily?.controlledExpressionBytes !== 564413140
...
if (
  snapshots.version !== "2.5.0" || snapshots.denominatorReady !== false ||
...
const registry = {
  ...base,
  registry: { ...base.registry, version: "5.5.0", publishedAt: "2026-08-15" },
  sourceFamilies,
  sourceSnapshots,
...
if (
  registry.works.length !== 1901 ||
  registry.works.flatMap((work) => work.expressions).length !== 2119 ||
  new Set(registry.works.map((work) => work.id)).size !== registry.works.length
) throw new Error("跨语种登记册 v5.5.0 作品或文本表达统计不一致");
const registryRaw = `${JSON.stringify(registry, null, 2)}\n`;
const checksumRaw = [
  `${sha256(registryRaw)}  registry-v5.5.0.json`,
  ...entries.slice(1).map(([, relativePath, raw]) => `${sha256(raw)}  ${relativePath.split("/").at(-1)}`),
].join("\n") + "\n";

if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== registryRaw) throw new Error("registry-v5.5.0.json 不可复现");
  if (await readFile(checksumPath, "utf8") !== checksumRaw) throw new Error("checksums-v5.5.0.sha256 不可复现");
  console.log("跨语种登记册 v5.5.0 可复现：T37 21/21 固定来源已完成净土、胜鬘与涅槃根经、直接注疏、再注释、集解及同题异作边界审计；全球作品分母保持未知。");
} else {
  await writeFile(outputPath, registryRaw, "utf8");
  await writeFile(checksumPath, checksumRaw, "utf8");
  console.log("跨语种登记册 v5.5.0 已生成：T37 新增 21 个净土、胜鬘与涅槃经疏作品及 21 个完整表达；根经、直接注疏、再注释、集解和同题异作保持可审计边界。");
}
