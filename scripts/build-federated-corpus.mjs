...
const inputs = {
  ...
  snapshots: "data/gbcr/source-snapshots-v1.6.0.json",
  ...
  cbetaBatch: "data/corpus/cbeta/batch-v3.5.0.json",
  cbetaCatalog: "data/corpus/cbeta/catalog-v3.5.0.json",
  cbetaManifest: "data/corpus/cbeta/manifest-v3.5.0.json",
  cbetaRegistry: "data/gbcr/registry-cbeta-v3.5.0.json",
...
const outputPath = resolve(root, "data/gbcr/registry-v4.6.0.json");
...
if (
  cbetaBatch.version !== "3.5.0" || cbetaBatch.files.length !== 12 ||
  cbetaBatch.collection.sourceRecordDenominator !== 12 ||
  cbetaBatch.collection.previouslyControlledSourceRecords !== 0 ||
  cbetaBatch.collection.controlledSourceRecords !== 12 ||
  cbetaBatch.collection.newSourceBytes !== 14447693 ||
  cbetaBatch.collection.newStableSegments !== 85917 ||
  cbetaBatch.collection.newFolios !== 3081 ||
  cbetaBatch.collection.newFullSourceTexts !== 11 ||
  cbetaBatch.collection.newPartialSourceWitnesses !== 1 ||
  cbetaBatch.collection.verifiedSameWorkExpressions !== 2 ||
  cbetaBatch.collection.verifiedPartialWorkWitnesses !== 1 ||
  cbetaBatch.collection.verifiedEditionWitnesses !== 0 ||
  cbetaBatch.collection.provisionalRecords !== 0 ||
  cbetaBatch.collection.newWorks !== 10 ||
  cbetaCatalog.files.length !== 1622 || cbetaManifest.files.length !== 1622 ||
  cbetaRegistry.registry.version !== "3.5.0" || cbetaRegistry.works.length !== 1425 ||
  cbetaRegistry.works.flatMap((work) => work.expressions).length !== 1622
) throw new Error("CBETA T28 固定批次、目录、清单或登记册统计不一致");

const cbetaFamily = cbetaRegistry.sourceFamilies.find((family) => family.id === "cbeta_chinese");
if (
  cbetaFamily?.controlledExpressionRecords !== 1636 ||
  cbetaFamily?.controlledExpressionBytes !== 456464713
...
if (
  snapshots.version !== "1.6.0" || snapshots.denominatorReady !== false ||
...
const registry = {
  ...base,
  registry: { ...base.registry, version: "4.6.0", publishedAt: "2026-08-14" },
  sourceFamilies,
  sourceSnapshots,
...
if (
  registry.works.length !== 1714 ||
  registry.works.flatMap((work) => work.expressions).length !== 1911 ||
  new Set(registry.works.map((work) => work.id)).size !== registry.works.length
) throw new Error("跨语种登记册 v4.6.0 作品或文本表达统计不一致");
const registryRaw = `${JSON.stringify(registry, null, 2)}\n`;
const checksumRaw = [
  `${sha256(registryRaw)}  registry-v4.6.0.json`,
  ...entries.slice(1).map(([, relativePath, raw]) => `${sha256(raw)}  ${relativePath.split("/").at(-1)}`),
].join("\n") + "\n";

if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== registryRaw) throw new Error("registry-v4.6.0.json 不可复现");
  if (await readFile(checksumPath, "utf8") !== checksumRaw) throw new Error("checksums-v4.6.0.sha256 不可复现");
  console.log("跨语种登记册 v4.6.0 可复现：T28 12/12 固定来源已完成作品、表达、旧译残存见证、扩释、同本异译与归属边界审计；全球作品分母保持未知。");
} else {
  await writeFile(outputPath, registryRaw, "utf8");
  await writeFile(checksumPath, checksumRaw, "utf8");
  console.log("跨语种登记册 v4.6.0 已生成：T28 新增 10 个作品、12 个表达或见证；《大毘婆沙》旧译残存见证、心论体系与《五事论》同本异译保持可审计边界。");
}
