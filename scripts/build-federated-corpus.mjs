...
const inputs = {
  base: "data/gbcr/registry-v1.4.0.json",
  snapshots: "data/gbcr/source-snapshots-v0.2.1.json",
  inventory: "data/gbcr/cbeta-taisho-sutra-inventory-v0.2.1.json",
  cbetaBatch: "data/corpus/cbeta/batch-v1.5.0.json",
  cbetaCatalog: "data/corpus/cbeta/catalog-v1.5.0.json",
  cbetaManifest: "data/corpus/cbeta/manifest-v1.5.0.json",
  cbetaRegistry: "data/gbcr/registry-cbeta-v1.5.0.json",
...
const outputPath = resolve(root, "data/gbcr/registry-v1.5.0.json");
...
if (
  cbetaBatch.version !== "1.5.0" || cbetaBatch.files.length !== 39 ||
  cbetaBatch.collection.sourceRecordDenominator !== 57 ||
  cbetaBatch.collection.controlledSourceRecords !== 57 ||
  cbetaBatch.collection.newSourceBytes !== 11312892 ||
  cbetaBatch.collection.newStableSegments !== 60230 ||
  cbetaCatalog.files.length !== 286 || cbetaManifest.files.length !== 286 ||
  cbetaRegistry.registry.version !== "1.5.0" || cbetaRegistry.works.length !== 271 ||
  cbetaRegistry.works.flatMap((work) => work.expressions).length !== 286
) throw new Error("CBETA T05–T08 固定批次、目录、清单或登记册统计不一致");

const cbetaFamily = cbetaRegistry.sourceFamilies.find((family) => family.id === "cbeta_chinese");
if (
  cbetaFamily?.controlledExpressionRecords !== 299 ||
  cbetaFamily?.controlledExpressionBytes !== 142623438
...
const registry = {
  ...base,
  registry: { ...base.registry, version: "1.5.0", publishedAt: "2026-08-12" },
  sourceFamilies,
  works: [...nonCbetaWorks, ...cbetaWorks],
};
if (
  registry.works.length !== 544 ||
  registry.works.flatMap((work) => work.expressions).length !== 559 ||
  new Set(registry.works.map((work) => work.id)).size !== registry.works.length
) throw new Error("跨语种登记册 v1.5.0 作品或文本表达统计不一致");
const registryRaw = `${JSON.stringify(registry, null, 2)}\n`;
const checksumRaw = [
  `${sha256(registryRaw)}  registry-v1.5.0.json`,
  ...entries.slice(1).map(([, relativePath, raw]) => `${sha256(raw)}  ${relativePath.split("/").at(-1)}`),
].join("\n") + "\n";

if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== registryRaw) throw new Error("registry-v1.5.0.json 不可复现");
  if (await readFile(checksumPath, "utf8") !== checksumRaw) throw new Error("checksums-v1.5.0.sha256 不可复现");
  console.log("跨语种登记册 v1.5.0 可复现：544 个作品实体、559 个完整文本表达。");
} else {
  await writeFile(outputPath, registryRaw, "utf8");
  await writeFile(checksumPath, checksumRaw, "utf8");
  console.log("跨语种登记册 v1.5.0 已生成：CBETA T05–T08 固定来源记录完成 57/57。");
}
