...
const inputs = {
  ...
  snapshots: "data/gbcr/source-snapshots-v1.9.0.json",
  ...
  cbetaBatch: "data/corpus/cbeta/batch-v3.8.0.json",
  cbetaCatalog: "data/corpus/cbeta/catalog-v3.8.0.json",
  cbetaManifest: "data/corpus/cbeta/manifest-v3.8.0.json",
  cbetaRegistry: "data/gbcr/registry-cbeta-v3.8.0.json",
...
const outputPath = resolve(root, "data/gbcr/registry-v4.9.0.json");
...
if (
  cbetaBatch.version !== "3.8.0" || cbetaBatch.files.length !== 43 ||
  cbetaBatch.collection.sourceRecordDenominator !== 43 ||
  cbetaBatch.collection.previouslyControlledSourceRecords !== 0 ||
  cbetaBatch.collection.controlledSourceRecords !== 43 ||
  cbetaBatch.collection.newSourceBytes !== 11039332 ||
  cbetaBatch.collection.newStableSegments !== 76949 ||
  cbetaBatch.collection.newFolios !== 2799 ||
  cbetaBatch.collection.newJuans !== 164 ||
  cbetaBatch.collection.newFullSourceTexts !== 43 ||
  cbetaBatch.collection.newPartialSourceWitnesses !== 0 ||
  cbetaBatch.collection.verifiedSameWorkExpressions !== 21 ||
  ...
  cbetaBatch.collection.newWorks !== 31 ||
  cbetaCatalog.files.length !== 1692 || cbetaManifest.files.length !== 1692 ||
  cbetaRegistry.registry.version !== "3.8.0" || cbetaRegistry.works.length !== 1481 ||
  cbetaRegistry.works.flatMap((work) => work.expressions).length !== 1692
...
if (
  cbetaFamily?.controlledExpressionRecords !== 1706 ||
  cbetaFamily?.controlledExpressionBytes !== 492461211
...
if (
  snapshots.version !== "1.9.0" || snapshots.denominatorReady !== false ||
...
const registry = {
  ...base,
  registry: { ...base.registry, version: "4.9.0", publishedAt: "2026-08-14" },
  sourceFamilies,
  sourceSnapshots,
  ...
  cbetaT31BoundaryAudit: {
    ...
    componentGroups: cbetaBatch.boundaryAudit.componentGroups,
...
if (
  registry.works.length !== 1770 ||
  registry.works.flatMap((work) => work.expressions).length !== 1981 ||
  new Set(registry.works.map((work) => work.id)).size !== registry.works.length
) throw new Error("跨语种登记册 v4.9.0 作品或文本表达统计不一致");
const registryRaw = `${JSON.stringify(registry, null, 2)}\n`;
const checksumRaw = [
  `${sha256(registryRaw)}  registry-v4.9.0.json`,
  ...entries.slice(1).map(([, relativePath, raw]) => `${sha256(raw)}  ${relativePath.split("/").at(-1)}`),
].join("\n") + "\n";

if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== registryRaw) throw new Error("registry-v4.9.0.json 不可复现");
  if (await readFile(checksumPath, "utf8") !== checksumRaw) throw new Error("checksums-v4.9.0.sha256 不可复现");
  console.log("跨语种登记册 v4.9.0 可复现：T31 43/43 固定来源已完成异译、根本论、释论、组成部分、异本与归属边界审计；全球作品分母保持未知。");
} else {
  await writeFile(outputPath, registryRaw, "utf8");
  await writeFile(checksumPath, checksumRaw, "utf8");
  console.log("跨语种登记册 v4.9.0 已生成：T31 新增 31 个作品、43 个表达或见证；异译、根本论、释论、组成部分、异本与归属争议保持可审计边界。");
}
