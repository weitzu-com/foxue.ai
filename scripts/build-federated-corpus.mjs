...
const inputs = {
  base: "data/gbcr/registry-v1.7.0.json",
  snapshots: "data/gbcr/source-snapshots-v0.2.1.json",
  inventory: "data/gbcr/cbeta-taisho-sutra-inventory-v0.2.1.json",
  cbetaBatch: "data/corpus/cbeta/batch-v1.8.0.json",
  cbetaCatalog: "data/corpus/cbeta/catalog-v1.8.0.json",
  cbetaManifest: "data/corpus/cbeta/manifest-v1.8.0.json",
  cbetaRegistry: "data/gbcr/registry-cbeta-v1.8.0.json",
...
const outputPath = resolve(root, "data/gbcr/registry-v1.8.0.json");
...
if (
  cbetaBatch.version !== "1.8.0" || cbetaBatch.files.length !== 11 ||
  cbetaBatch.collection.sourceRecordDenominator !== 12 ||
  cbetaBatch.collection.controlledSourceRecords !== 12 ||
  cbetaBatch.collection.newSourceBytes !== 3417170 ||
  cbetaBatch.collection.newStableSegments !== 24936 ||
  cbetaCatalog.files.length !== 342 || cbetaManifest.files.length !== 342 ||
  cbetaRegistry.registry.version !== "1.8.0" || cbetaRegistry.works.length !== 310 ||
  cbetaRegistry.works.flatMap((work) => work.expressions).length !== 342
) throw new Error("CBETA T11 固定批次、目录、清单或登记册统计不一致");

const cbetaFamily = cbetaRegistry.sourceFamilies.find((family) => family.id === "cbeta_chinese");
if (
  cbetaFamily?.controlledExpressionRecords !== 355 ||
  cbetaFamily?.controlledExpressionBytes !== 161714389
...
const registry = {
  ...base,
  registry: { ...base.registry, version: "1.8.0", publishedAt: "2026-08-12" },
  sourceFamilies,
  works: [...nonCbetaWorks, ...cbetaWorks],
};
if (
  registry.works.length !== 583 ||
  registry.works.flatMap((work) => work.expressions).length !== 615 ||
  new Set(registry.works.map((work) => work.id)).size !== registry.works.length
) throw new Error("跨语种登记册 v1.8.0 作品或文本表达统计不一致");
const registryRaw = `${JSON.stringify(registry, null, 2)}\n`;
const checksumRaw = [
  `${sha256(registryRaw)}  registry-v1.8.0.json`,
  ...entries.slice(1).map(([, relativePath, raw]) => `${sha256(raw)}  ${relativePath.split("/").at(-1)}`),
].join("\n") + "\n";

if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== registryRaw) throw new Error("registry-v1.8.0.json 不可复现");
  if (await readFile(checksumPath, "utf8") !== checksumRaw) throw new Error("checksums-v1.8.0.sha256 不可复现");
  console.log("跨语种登记册 v1.8.0 可复现：583 个作品实体、615 个文本表达或见证。");
} else {
  await writeFile(outputPath, registryRaw, "utf8");
  await writeFile(checksumPath, checksumRaw, "utf8");
  console.log("跨语种登记册 v1.8.0 已生成：CBETA T11 固定来源记录完成 12/12。");
}
