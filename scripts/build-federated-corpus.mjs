...
const inputs = {
  base: "data/gbcr/registry-v0.7.0.json",
...
const outputPath = resolve(root, "data/gbcr/registry-v0.8.0.json");
...
const dighaWorks = dighaManifest.files.map((file) => ({
  id: file.workId,
  ...
  relationDecision: "作为巴利《长部》中的独立经文登记；未经学术复核，不自动声称与汉译阿含或其他语种文本逐段对应。",
  expressions: [
    {
...
const sourceFamilies = base.sourceFamilies.map((family) =>
  family.id === "suttacentral_early_buddhist_texts"
    ? {
        ...
        controlledWorks: 35,
        controlledExpressions: 35,
        controlledRootRecords: 60,
        controlledRootBytes: 1920173,
        denominatorWorks: null,
        denominatorNote: "7,288 是固定提交中的巴利 root 候选记录；当前 60 个物理文件组成《法句经》与《长部》34 经，文件数不是全球作品分母。",
      }
    : family,
);
...
const registry = {
  ...base,
  registry: { ...base.registry, version: "0.8.0", publishedAt: "2026-08-11" },
  sourceFamilies,
  works: [...base.works, ...dighaWorks],
};
const registryRaw = `${JSON.stringify(registry, null, 2)}\n`;
const checksumRaw = [
  `${sha256(registryRaw)}  registry-v0.8.0.json`,
  ...entries.slice(1).map(([, relativePath, raw]) => `${sha256(raw)}  ${relativePath.split("/").at(-1)}`),
].join("\n") + "\n";

if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== registryRaw) throw new Error("registry-v0.8.0.json 不可复现");
  if (await readFile(checksumPath, "utf8") !== checksumRaw) throw new Error("checksums-v0.8.0.sha256 不可复现");
  console.log("跨语种登记册 v0.8.0 可复现：56 部作品、60 个文本表达。");
} else {
  await writeFile(outputPath, registryRaw, "utf8");
  await writeFile(checksumPath, checksumRaw, "utf8");
  console.log("跨语种登记册 v0.8.0 已生成：新增巴利《长部》34 经。");
}
