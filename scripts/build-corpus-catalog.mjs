...
const outputVersion = "4.23.0";
const catalogPath = resolve(root, "data/corpus/cbeta/catalog-v4.23.0.json");
...
const snapshotPath = resolve(root, "data/gbcr/source-snapshots-v4.4.0.json");
...
const t49Batch = JSON.parse(await readFile(t49BatchPath, "utf8"));
const t50Batch = JSON.parse(await readFile(t50BatchPath, "utf8"));
const t51Batch = JSON.parse(await readFile(t51BatchPath, "utf8"));
const t52Batch = JSON.parse(await readFile(t52BatchPath, "utf8"));
const t53Batch = JSON.parse(await readFile(t53BatchPath, "utf8"));
const t54Batch = JSON.parse(await readFile(t54BatchPath, "utf8"));
const t55Batch = JSON.parse(await readFile(t55BatchPath, "utf8"));
const t85Batch = JSON.parse(await readFile(t85BatchPath, "utf8"));
...
const cbetaT49SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_buddhist_histories_sectarian_records_t49",
);
const cbetaT50SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_buddhist_biographies_hagiographies_t50",
);
const cbetaT51SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_pilgrimage_lineage_travel_gazetteers_t51",
);
const cbetaT52SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_buddhist_apologetics_debate_memorials_t52",
);
const cbetaT53SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_buddhist_encyclopedic_compendia_t53",
);
const cbetaT54SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_buddhist_reference_and_non_buddhist_sources_t54",
);
const cbetaT55SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
  (subset) => subset.id === "taisho_buddhist_catalogues_t55",
);
const cbetaT85SubsetSnapshot = cbetaSnapshotSource.candidateSubsets.find(
...
if (
  t49Inventory.source.commit !== catalog.source.commit ||
  t49Inventory.totals.records !== cbetaT49SubsetSnapshot?.candidateRecordCount ||
  t49Inventory.totals.upstreamBytes !== cbetaT49SubsetSnapshot?.candidateBytes ||
  sha256(t49InventoryRaw) !== cbetaT49SubsetSnapshot?.inventorySha256
) {
  throw new Error("T49 结集、法灭、部派论书与佛教史传逐文件清单与来源快照不一致");
}
if (
  t50Inventory.source.commit !== catalog.source.commit ||
  t50Inventory.totals.records !== cbetaT50SubsetSnapshot?.candidateRecordCount ||
  t50Inventory.totals.upstreamBytes !== cbetaT50SubsetSnapshot?.candidateBytes ||
  sha256(t50InventoryRaw) !== cbetaT50SubsetSnapshot?.inventorySha256
) {
  throw new Error("T50 佛传、论师传与僧尼史传逐文件清单与来源快照不一致");
}
if (
  t51Inventory.source.commit !== catalog.source.commit ||
  t51Inventory.totals.records !== cbetaT51SubsetSnapshot?.candidateRecordCount ||
  t51Inventory.totals.upstreamBytes !== cbetaT51SubsetSnapshot?.candidateBytes ||
  sha256(t51InventoryRaw) !== cbetaT51SubsetSnapshot?.inventorySha256
) {
  throw new Error("T51 求法传、感应传、灯录、游记与佛教方志逐文件清单与来源快照不一致");
}
if (
  t52Inventory.source.commit !== catalog.source.commit ||
  t52Inventory.totals.records !== cbetaT52SubsetSnapshot?.candidateRecordCount ||
  t52Inventory.totals.upstreamBytes !== cbetaT52SubsetSnapshot?.candidateBytes ||
  sha256(t52InventoryRaw) !== cbetaT52SubsetSnapshot?.inventorySha256
) {
  throw new Error("T52 护法论辩、三教交涉、感通录与表制文书逐文件清单与来源快照不一致");
}
if (
  t53Inventory.source.commit !== catalog.source.commit ||
  t53Inventory.totals.records !== cbetaT53SubsetSnapshot?.candidateRecordCount ||
  t53Inventory.totals.upstreamBytes !== cbetaT53SubsetSnapshot?.candidateBytes ||
  sha256(t53InventoryRaw) !== cbetaT53SubsetSnapshot?.inventorySha256
) {
  throw new Error("T53 佛教类书逐文件清单与来源快照不一致");
}
if (
  t54Inventory.source.commit !== catalog.source.commit ||
  t54Inventory.totals.records !== cbetaT54SubsetSnapshot?.candidateRecordCount ||
  t54Inventory.totals.upstreamBytes !== cbetaT54SubsetSnapshot?.candidateBytes ||
  sha256(t54InventoryRaw) !== cbetaT54SubsetSnapshot?.inventorySha256
) {
  throw new Error("T54 佛教参考资料与外教文献逐文件清单与来源快照不一致");
}
if (
  t55Inventory.source.commit !== catalog.source.commit ||
  t55Inventory.totals.records !== cbetaT55SubsetSnapshot?.candidateRecordCount ||
  t55Inventory.totals.upstreamBytes !== cbetaT55SubsetSnapshot?.candidateBytes ||
  sha256(t55InventoryRaw) !== cbetaT55SubsetSnapshot?.inventorySha256
) {
  throw new Error("T55 佛教经录、请来目录与章疏书目逐文件清单与来源快照不一致");
}
if (
  t85Inventory.source.commit !== catalog.source.commit ||
  t85Inventory.totals.records !== cbetaT85SubsetSnapshot?.candidateRecordCount ||
  t85Inventory.totals.upstreamBytes !== cbetaT85SubsetSnapshot?.candidateBytes ||
  sha256(t85InventoryRaw) !== cbetaT85SubsetSnapshot?.inventorySha256
) {
  throw new Error("T85 古逸部与疑似部逐文件清单与来源快照不一致");
}
...
const t49InventoryByPath = new Map(t49Inventory.records.map((record) => [record.upstreamPath, record]));
const t50InventoryByPath = new Map(t50Inventory.records.map((record) => [record.upstreamPath, record]));
const t51InventoryByPath = new Map(t51Inventory.records.map((record) => [record.upstreamPath, record]));
const t52InventoryByPath = new Map(t52Inventory.records.map((record) => [record.upstreamPath, record]));
const t53InventoryByPath = new Map(t53Inventory.records.map((record) => [record.upstreamPath, record]));
const t54InventoryByPath = new Map(t54Inventory.records.map((record) => [record.upstreamPath, record]));
const t55InventoryByPath = new Map(t55Inventory.records.map((record) => [record.upstreamPath, record]));
const t85InventoryByPath = new Map(t85Inventory.records.map((record) => [record.upstreamPath, record]));
...
for (const entry of catalog.files) {
  ...
  const work = worksById.get(entry.workId) ?? {
    id: entry.workId,
    workType: entry.canonicalStatus === "non_buddhist_reference_text"
      ? "non_buddhist_reference_text"
      : entry.workIdentityStatus?.startsWith("provisional_")
        ? "provisional_bibliographic_entity"
        : "canonical_text",
    ...(entry.canonicalStatus ? { canonicalStatus: entry.canonicalStatus } : {}),
    ...(entry.buddhaWordStatus ? { buddhaWordStatus: entry.buddhaWordStatus } : {}),
    ...
    ...(entry.workIdentityStatus?.startsWith("provisional_") ? {
      relationDecision: "暂按单一来源记录建立可追踪书目实体；异译、别本、同题写本、平行经与跨语种作品关系尚待校勘，不据此声称已经完成作品级去重。",
    } : {}),
    expressions: []
  };
...
const sourceFamilies = previousRegistry.sourceFamilies.map((family) => family.id === "cbeta_chinese"
  ? {
      ...
      t49SourceRecordDenominator: t49Batch.collection.sourceRecordDenominator,
      t49ControlledSourceRecords: t49Batch.collection.controlledSourceRecords,
      t49SourceRecordPercentage: 100,
      t50SourceRecordDenominator: t50Batch.collection.sourceRecordDenominator,
      t50ControlledSourceRecords: t50Batch.collection.controlledSourceRecords,
      t50SourceRecordPercentage: 100,
      t51SourceRecordDenominator: t51Batch.collection.sourceRecordDenominator,
      t51ControlledSourceRecords: t51Batch.collection.controlledSourceRecords,
      t51SourceRecordPercentage: 100,
      t52SourceRecordDenominator: t52Batch.collection.sourceRecordDenominator,
      t52ControlledSourceRecords: t52Batch.collection.controlledSourceRecords,
      t52SourceRecordPercentage: 100,
      t53SourceRecordDenominator: t53Batch.collection.sourceRecordDenominator,
      t53ControlledSourceRecords: t53Batch.collection.controlledSourceRecords,
      t53SourceRecordPercentage: 100,
      t54SourceRecordDenominator: t54Batch.collection.sourceRecordDenominator,
      t54ControlledSourceRecords: t54Batch.collection.controlledSourceRecords,
      t54SourceRecordPercentage: 100,
      t55SourceRecordDenominator: t55Batch.collection.sourceRecordDenominator,
      t55ControlledSourceRecords: t55Batch.collection.controlledSourceRecords,
      t55SourceRecordPercentage: 100,
      t85SourceRecordDenominator: t85Batch.collection.sourceRecordDenominator,
      t85ControlledSourceRecords: t85Batch.collection.controlledSourceRecords,
      t85SourceRecordPercentage: 100,
      denominatorWorks: null,
      denominatorNote: "2,471 是大正藏 T01–T55 与 T85 四十个固定候选子集中的来源记录，不是去重后的全球作品数。T01–T55 各固定子集均已闭合，T85 完成 192/192。T54 的 9 份外教参照文本排除于佛教经典与佛陀亲说覆盖分子；T55 的 42 份目录学原典、T85 的 135 份古逸记录与 57 份疑似部记录均不标为佛陀逐字亲说。固定来源记录完整性不能偷换成全球作品覆盖率。"
    }
  : family);
const registry = {
  ...previousRegistry,
