...
const outputVersion = "3.3.0";
const catalogPath = resolve(root, "data/corpus/cbeta/catalog-v3.3.0.json");
...
const snapshotPath = resolve(root, "data/gbcr/source-snapshots-v1.4.0.json");
...
const cbetaCandidateSubsets = snapshots.sources
  ...
  ?.candidateSubsets?.filter((subset) => ["taisho_chinese_sutra_t01_t17", "taisho_esoteric_t18", "taisho_esoteric_t19", "taisho_esoteric_t20", "taisho_esoteric_t21", "taisho_vinaya_t22", "taisho_vinaya_t23", "taisho_vinaya_t24", "taisho_sutra_commentary_t25", "taisho_sutra_commentary_abhidharma_t26"].includes(subset.id));
if (cbetaCandidateSubsets?.length !== 10) throw new Error("缺少汉译经藏、T18–T21 密教部、T22–T24 律部、T25 释经论部或 T26 释经论与毘昙部候选子集快照");
const controlledSubsetFiles = files
  .flatMap(sourceUnits)
  .filter((file) => /^T\/T(0[1-9]|1[0-9]|2[0-6])\//.test(file.upstreamPath));
const controlledSubsetRecords = controlledSubsetFiles.length;
for (const file of controlledSubsetFiles) {
  const inventoryRecord = inventoryByPath.get(file.upstreamPath) ?? t18InventoryByPath.get(file.upstreamPath) ?? t19InventoryByPath.get(file.upstreamPath) ?? t20InventoryByPath.get(file.upstreamPath) ?? t21InventoryByPath.get(file.upstreamPath) ?? t22InventoryByPath.get(file.upstreamPath) ?? t23InventoryByPath.get(file.upstreamPath) ?? t24InventoryByPath.get(file.upstreamPath) ?? t25InventoryByPath.get(file.upstreamPath) ?? t26InventoryByPath.get(file.upstreamPath);
  if (
    !inventoryRecord ||
...
const sourceFamilies = previousRegistry.sourceFamilies.map((family) => family.id === "cbeta_chinese"
  ? {
      ...
      denominatorNote: "1,623 是大正藏 T01–T26 十个固定候选子集中的来源记录，不是去重后的全球作品数。T01–T17 已完成 881/881，T18 完成 76/76，T19 完成 126/126，T20 完成 184/184，T21 完成 228/228，T22 完成 15/15，T23 完成 13/13，T24 完成 59/59，T25 完成 15/15，T26 完成 26/26。T18–T21 密教部、T22–T24 律部、T25–T26 释经论与毘昙部分别保留译经、仪轨、论造、编集、广律、戒本、羯磨、根本经论、复注、疑伪归属、异译、异传和无署名释文边界；目录部类、传统作者、题名或机器相似度不等于同一作品或佛陀逐字亲说归属。"
    }
  : family);
const registry = {
  ...previousRegistry,
