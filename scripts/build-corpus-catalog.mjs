...
const outputVersion = "4.1.0";
const catalogPath = resolve(root, "data/corpus/cbeta/catalog-v4.1.0.json");
...
const snapshotPath = resolve(root, "data/gbcr/source-snapshots-v2.2.0.json");
...
if (cbetaCandidateSubsets?.length !== 18) throw new Error("缺少汉译经藏、T18–T21 密教部、T22–T24 律部或 T25–T34 释经论、毘昙、中观、瑜伽、论集与经疏部候选子集快照");
const controlledSubsetFiles = files
  .flatMap(sourceUnits)
  .filter((file) => /^T\/T(0[1-9]|[12][0-9]|3[0-4])\//.test(file.upstreamPath));
...
const sourceFamilies = previousRegistry.sourceFamilies.map((family) => family.id === "cbeta_chinese"
  ? {
      ...
      denominatorNote: "1,810 是大正藏 T01–T34 十八个固定候选子集中的来源记录，不是去重后的全球作品数。T01–T17 已完成 881/881，T18 完成 76/76，T19 完成 126/126，T20 完成 184/184，T21 完成 228/228，T22 完成 15/15，T23 完成 13/13，T24 完成 59/59，T25 完成 15/15，T26 完成 26/26，T27 完成 1/1，T28 完成 12/12，T29 完成 6/6，T30 完成 21/21，T31 完成 43/43，T32 完成 66/66，T33 完成 25/25，T34 完成 13/13。T18–T21 密教部、T22–T24 律部、T25–T34 释经论、毘昙、中观、瑜伽、论集与经疏部分别保留译经、仪轨、论造、编集、广律、戒本、羯磨、根本颂、释论、完整论书、部分作品见证、组成部分、节略与分离见证、同本异译、音写、失译、汉地撰成、经疏、讲说记录、章节注释、再注释与争议归属边界；目录部类、传统作者、题名或机器相似度不等于同一作品或佛陀逐字亲说归属。"
    }
  : family);
const registry = {
  ...previousRegistry,
