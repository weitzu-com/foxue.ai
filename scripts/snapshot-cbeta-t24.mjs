...
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t24.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
...
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T24"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T24 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T24\/T24n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 59) throw new Error(`T24 固定来源记录应为 59，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 19745486) {
  throw new Error("T24 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_vinaya_t24",
    label: "大正藏 T24 律部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T24 目录下、文件名卷号一致的 T24n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "律部含根本说一切有部律事、律摄、戒经、羯磨、威仪、在家戒与菩萨戒，也含同经异本、异译、节出、失译、无署名及传统归属有争议的中国撰述候选；来源文件数不是去重作品数，也不是全球佛典分母。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T24n/, "T"),
      volume: "T24",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v1.1.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T24 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t24-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T24: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T24 Git tree 不一致`);
    }
  }
  console.log(`CBETA T24 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T24 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
