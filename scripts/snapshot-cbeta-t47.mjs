...
const version = "0.1.0";
const snapshotVersion = "3.5.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "62f81f468f7ff7c88a078cb3aef9cfcd9e2af7e7";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t47.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T47"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T47 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T47"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T47 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T47\/T47n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 49) throw new Error(`T47 固定来源记录应为 49，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 11178014) {
  throw new Error("T47 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_pure_land_chan_records_t47",
    label: "大正藏 T47 净土论著、礼赞仪轨与禅宗语录固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T47 目录下、文件名卷号一致的 T47n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T47 的 49 份来源记录横跨净土论著、文类汇编、礼赞与忏愿仪轨，以及禅宗语录。DILA 为 49 份记录分配 49 个不同作品权威号；T1969A/B、T1986A/B、T1987A/B、T1994A/B、T1998A/B 虽共享数字经号或同一宗师/编者脉络，仍有不同题名、责任、范围与权威号。共同宗派、宗师、作者、仪式功能、引文或高文本重叠均不能自动合并作品或表达，也不能把东亚撰述标成佛陀逐字亲说。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T47n/, "T"),
      volume: "T47",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v3.4.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T47 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t47-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T47: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T47 Git tree 不一致`);
    }
  }
  console.log(`CBETA T47 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T47 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
