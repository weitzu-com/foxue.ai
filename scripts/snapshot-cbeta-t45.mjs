...
const version = "0.1.0";
const snapshotVersion = "3.3.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "283aac5b46cfaf8d5cc1b1e95cfb6be07ca3af18";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t45.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T45"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T45 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T45"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T45 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T45\/T45n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 61) throw new Error(`T45 固定来源记录应为 61，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 11260895) {
  throw new Error("T45 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_east_asian_schools_vinaya_rituals_t45",
    label: "大正藏 T45 三论、法相、华严、律仪与忏法固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T45 目录下、文件名卷号一致的 T45n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T45 的 61 份来源记录横跨三论与肇论传统、法相宗义章、华严宗著述、律仪和忏法。根本文本、直接论疏、再注释、同作者异作、复合责任题记、同经号 a/b 或 A/B 文本、仪轨和集撰忏法必须分层；同卷、同宗、同题、共同作者、引文或高文本重叠均不能把独立作品、表达和注释层自动合并，也不能把东亚撰述标成佛陀逐字亲说。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T45n/, "T"),
      volume: "T45",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v3.2.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T45 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t45-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T45: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T45 Git tree 不一致`);
    }
  }
  console.log(`CBETA T45 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T45 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
