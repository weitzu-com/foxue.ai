...
const batchPath = resolve(root, "data/corpus/cbeta/batch-v4.22.0.json");
const outputPath = resolve(root, "data/corpus/cbeta/catalog-v4.22.0.json");
...
const additions = batch.files.map((file) => {
  ...
  const numericJuans = juanSequence ?? Array.from({ length: last - first + 1 }, (_, index) => first + index);
  if (
    !Array.isArray(numericJuans) || numericJuans.length === 0 || numericJuans[0] !== first ||
    numericJuans.at(-1) !== last || numericJuans.some((juan, index) =>
      !Number.isSafeInteger(juan) || juan < 1 || (index > 0 && juan <= numericJuans[index - 1]))
  ) throw new Error(`${file.id} 原始卷序无效`);
  return {
    ...file,
    verification: {
      segments: verification.segments,
      folios: verification.folios,
      juans: numericJuans.map((juan) => String(juan).padStart(3, "0")),
  ...
  ...base.files.map((file) => {
    ...
    return {
      ...
      ...(override?.bibliographicRelations?.length ? {
        bibliographicRelations: [...(file.bibliographicRelations ?? []), ...override.bibliographicRelations],
      } : {}),
    };
  }),
  ...additions,
