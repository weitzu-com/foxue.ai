...
const batch = JSON.parse(await readFile(resolve(root, "data/corpus/suttacentral/mn-batch-v0.9.0.json"), "utf8"));
const requested = process.argv.slice(2);
const selected = requested.includes("--all")
  ? batch.files
  : batch.files.filter((file) => requested.includes(file.id));
if (selected.length === 0) {
  console.error("用法：pnpm import:suttacentral:mn --all，或指定 MN1…MN152");
  process.exit(1);
}
