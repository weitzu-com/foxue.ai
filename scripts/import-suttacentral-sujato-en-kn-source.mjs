...
const batch = JSON.parse(
  await readFile(resolve(root, "data/corpus/suttacentral/sujato-en-kn-batch-v1.0.0.json"), "utf8"),
);
...
if (selected.length === 0) {
  console.error("用法：pnpm import:suttacentral:sujato-en-kn --all，或指定 EN-ITI1…");
  process.exit(1);
}
...
console.log(`Sujato 小部英译 ${selected.length} 个固定文件已核验。`);
