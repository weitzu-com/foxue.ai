...
const version = "0.1.0";
const snapshotVersion = "2.7.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "c6ce58da43d4d0d4376bfd07e3f39215f4e8b8ae";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t39.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T39"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T39 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T39"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T39 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T39\/T39n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 21) throw new Error(`T39 固定来源记录应为 21，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 12940653) {
  throw new Error("T39 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_golden_light_lankavatara_esoteric_commentary_t39",
    label: "大正藏 T39 金光明、楞伽及显密经疏部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T39 目录下、文件名卷号一致的 T39n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T39 的 21 份来源记录是《金光明经》《楞伽经》及盂兰盆、温室、四十二章、圆觉、大日、金刚顶、楞严、请观音和陀罗尼经的东亚玄义、文句、经疏、注解、略疏、义诀、集注与再注释；共同根经、同一作者、章节范围、经轨关系和传统归属必须分层计量，不能把注疏目录位置、密教仪轨或正文引文误作佛陀逐字亲说归属。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T39n/, "T"),
      volume: "T39",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v2.6.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T39 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t39-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T39: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T39 Git tree 不一致`);
    }
  }
  console.log(`CBETA T39 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T39 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
