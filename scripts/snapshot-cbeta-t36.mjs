...
const version = "0.1.0";
const snapshotVersion = "2.4.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "d8f490484131832ad046947dfa690483434ce5d7";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t36.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T36"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T36 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T36"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T36 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T36\/T36n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 8) throw new Error(`T36 固定来源记录应为 8，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 14520834) {
  throw new Error("T36 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_huayan_commentary_t36",
    label: "大正藏 T36 华严经疏部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T36 目录下、文件名卷号一致的 T36n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T36 的 8 份来源记录是围绕八十卷本《大方广佛华严经》及澄观疏形成的再注释、略策、章释、论、卷意、决疑论、观门骨目与宫廷讲义；根本经、直接注释、再注释、同作者异作和不同体例必须分层计量，不能把经疏目录位置或同根经关系误作佛陀逐字亲说归属。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T36n/, "T"),
      volume: "T36",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v2.3.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T36 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t36-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T36: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T36 Git tree 不一致`);
    }
  }
  console.log(`CBETA T36 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T36 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
