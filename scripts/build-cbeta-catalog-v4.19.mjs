...
const batchPath = resolve(root, "data/corpus/cbeta/batch-v4.19.0.json");
const outputPath = resolve(root, "data/corpus/cbeta/catalog-v4.19.0.json");
...
const additions = batch.files.map((file) => {
  ...
  const juans = Array.from({ length: last - first + 1 }, (_, index) => String(first + index).padStart(3, "0"));
  return {
    ...file,
    verification: {
