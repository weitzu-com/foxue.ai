...
const batchPath = resolve(root, "data/corpus/suttacentral/abhidhamma-batch-v1.5.0.json");
const outputPath = resolve(root, "data/corpus/suttacentral/abhidhamma-manifest-v1.5.0.json");
...
for (const work of batch.works) {
  ...
  requireValue(reading.omittedEmptySegmentIds.length === 0, `${work.id} 不应存在空段落`);
  ...
  files.push({
    ...
    presentation: {
      ...
      summary: `完整保存 ${work.sourceRecordCount} 份固定 root 记录、${work.stableSegments} 个 Bilara 原生稳定段落；固定来源未发现空正文值。`,
      sourceUrl: work.sourceUrl,
    },
    relationDecision: "按上座部论藏七论的书级边界登记为巴利表达；物理文件只作可复核章节或细分来源，不作为独立作品计数。",
    verification: {
...
if (process.argv.includes("--verify")) {
  requireValue(await readFile(outputPath, "utf8") === serialized, "SuttaCentral abhidhamma-manifest-v1.5.0.json 与确定性输出不一致");
  console.log(`SuttaCentral 巴利论藏 root v${batch.version} 可复现：${files.length} 个表达、${batch.collection.sourceRecordCount} 份来源、${batch.collection.stableSegments} 个稳定段落。`);
} else {
  await writeFile(outputPath, serialized, "utf8");
  console.log(`SuttaCentral 巴利论藏 root v${batch.version} 已生成：${files.length} 个表达、${batch.collection.sourceRecordCount} 份来源、${batch.collection.stableSegments} 个稳定段落。`);
}
