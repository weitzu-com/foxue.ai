...
import { parseBilaraAnguttaraSources } from "../src/lib/bilara-reading.mjs";

const root = process.cwd();
const batchPath = resolve(root, "data/corpus/suttacentral/an-batch-v1.1.0.json");
const outputPath = resolve(root, "data/corpus/suttacentral/an-manifest-v1.1.0.json");
...
if (filesByGroup.size !== batch.collection.groupCount) {
  throw new Error("SuttaCentral 《增支部》分组数量与固定批次不一致");
}
...
for (const [groupId, groupFiles] of filesByGroup) {
  ...
  const reading = parseBilaraAnguttaraSources(sources);
  ...
  if (
    reading.title !== `Aṅguttara Nikāya ${first.groupNumber}` ||
  ...
  manifestFiles.push({
    ...
    presentation: {
      title: first.titleZh,
      alternateTitle: `Aṅguttara Nikāya ${first.groupNumber}`,
      tradition: first.tradition,
      language: "巴利语（罗马字母）",
      canonRef: `SuttaCentral ${groupId}`,
      translator: first.edition,
      summary: `巴利《增支部》第 ${first.groupNumber} 集；${groupFiles.length} 个物理 root 记录连续表示 ${expectedRepresentedSuttas} 个经号，保留 Bilara 原生段落标识与缩略范围。`,
...
if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== serialized) {
    throw new Error("SuttaCentral an-manifest-v1.1.0.json 与固定批次确定性输出不一致");
  }
  console.log(`SuttaCentral 《增支部》 v${batch.version} 可复现：${manifestFiles.length} 个集、${batch.collection.recordCount} 个来源记录、${batch.collection.stableSegments} 个原生段落。`);
} else {
  await writeFile(outputPath, serialized, "utf8");
  console.log(`SuttaCentral 《增支部》 v${batch.version} 已生成：${manifestFiles.length} 个集、${batch.collection.recordCount} 个来源记录、${batch.collection.stableSegments} 个原生段落。`);
}
