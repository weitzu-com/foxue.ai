...
const batch = JSON.parse(await readFile(resolve(root, "data/corpus/suttacentral/dn-batch-v0.8.0.json"), "utf8"));
const requested = process.argv.slice(2);
const selected = requested.includes("--all")
  ? batch.files
  : batch.files.filter((file) => requested.includes(file.id));
if (selected.length === 0) {
  console.error("用法：pnpm import:suttacentral:dn --all，或指定 DN1…DN34");
  process.exit(1);
}
