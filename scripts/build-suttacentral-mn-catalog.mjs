...
const batchPath = resolve(root, "data/corpus/suttacentral/mn-batch-v0.9.0.json");
const outputPath = resolve(root, "data/corpus/suttacentral/mn-manifest-v0.9.0.json");
...
for (const file of batch.files) {
  ...
  manifestFiles.push({
    ...
    presentation: {
      ...
      summary: `巴利《中部》第 ${Number(file.suttaId.slice(2))} 经 ${file.titlePali}；保留 SuttaCentral Bilara 原生段落标识。`,
...
if (process.argv.includes("--verify")) {
  if (await readFile(outputPath, "utf8") !== serialized) {
    throw new Error("SuttaCentral mn-manifest-v0.9.0.json 与固定批次确定性输出不一致");
  }
  console.log(`SuttaCentral 《中部》 v${batch.version} 可复现：${manifestFiles.length} 部完整原文、${batch.collection.stableSegments} 个原生段落。`);
} else {
  await writeFile(outputPath, serialized, "utf8");
  console.log(`SuttaCentral 《中部》 v${batch.version} 已生成：${manifestFiles.length} 部完整原文、${batch.collection.stableSegments} 个原生段落。`);
}
