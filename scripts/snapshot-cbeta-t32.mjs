...
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t32.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
...
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T32"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T32 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T32\/T32n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 66) throw new Error(`T32 固定来源记录应为 66，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 11121227) {
  throw new Error("T32 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_sastra_t32",
    label: "大正藏 T32 论集部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T32 目录下、文件名卷号一致的 T32n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T32 的 66 份来源记录包含译论、注释、纲要、书信、发愿文、譬喻集、汉地撰述、疑伪或归属争议材料，以及同经号 A/B 见证；来源文件、表达、作品、版本、组成部分、完整性和传统作者题记必须分层计量。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T32n/, "T"),
      volume: "T32",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v1.9.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T32 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t32-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T32: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T32 Git tree 不一致`);
    }
  }
  console.log(`CBETA T32 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T32 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
