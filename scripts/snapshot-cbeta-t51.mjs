...
const version = "0.1.0";
const snapshotVersion = "3.9.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "3cd2df08990cd96232a6cfc41946d5147acc76f3";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t51.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T51"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T51 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T51"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T51 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T51\/T51n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 36) throw new Error(`T51 固定来源记录应为 36，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 17025589) {
  throw new Error("T51 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_pilgrimage_lineage_travel_gazetteers_t51",
    label: "大正藏 T51 求法传、感应传、灯录、游记与佛教方志固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T51 目录下、文件名卷号一致的 T51n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T51 的 36 份来源记录横跨求法高僧传、法华与净土往生传、华严感应传、禅宗灯录与付法史、冥报感应录、西域游记、寺塔记以及佛教名山方志。连续经号、共同信仰对象、传记或感应体裁、同作者、续修题名、共享故事、地理对象、引文或机器文本重叠均不能自动合并作品，也不能把后出史传和方志标成佛陀逐字亲说。作品权威号、责任、关系与文本范围仍须逐份审计。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T51n/, "T"),
      volume: "T51",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v3.8.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T51 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t51-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T51: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T51 Git tree 不一致`);
    }
  }
  console.log(`CBETA T51 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T51 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
