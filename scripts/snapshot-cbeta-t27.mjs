...
const version = "0.1.0";
const snapshotVersion = "1.5.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t27.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
...
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T27"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T27 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T27\/T27n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 1) throw new Error(`T27 固定来源记录应为 1，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 9552926) {
  throw new Error("T27 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_abhidharma_commentary_t27",
    label: "大正藏 T27 毘曇部《大毘婆沙論》固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T27 目录下、文件名卷号一致的 T27n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T27 只有 T1545《阿毘達磨大毘婆沙論》一份大型来源记录；一份来源记录、一个作品、二百卷与八万余稳定行段是不同计量单位，也不是佛陀亲说经文数或全球佛典分母。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T27n/, "T"),
      volume: "T27",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v1.4.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T27 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t27-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T27: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T27 Git tree 不一致`);
    }
  }
  console.log(`CBETA T27 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T27 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
