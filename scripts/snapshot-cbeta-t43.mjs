...
const version = "0.1.0";
const snapshotVersion = "3.1.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "85bd0982dfaf0d132a3a87dad555cab5571a6d9f";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t43.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T43"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T43 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T43"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T43 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T43\/T43n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 6) throw new Error(`T43 固定来源记录应为 6，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 10412626) {
  throw new Error("T43 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_yogacara_commentaries_t43",
    label: "大正藏 T43 瑜伽论疏部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T43 目录下、文件名卷号一致的 T43n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T43 的 6 份来源记录是窺基《瑜伽师地论略纂》《成唯识论述记》《成唯识论掌中枢要》《唯识二十论述记》、惠沼《成唯识论了义灯》和智周《成唯识论演秘》六部汉地唯识论疏。它们分别连接 T1579、T1585、T1590 与 T1830；直接注疏、枢要、再注释、同作者异作和不同根本论必须分层。共同唯识传统、师承、作者、题名或正文引文不能把六部作品误作根本论表达、彼此同作或佛陀逐字亲说。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T43n/, "T"),
      volume: "T43",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v3.0.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T43 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t43-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T43: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T43 Git tree 不一致`);
    }
  }
  console.log(`CBETA T43 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T43 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
