...
const version = "0.1.0";
const snapshotVersion = "3.7.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "6d6d2d6efa2209c764fb89653d3864c04ccb10e6";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t49.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T49"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T49 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T49"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T49 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T49\/T49n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 14) throw new Error(`T49 固定来源记录应为 14，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 13132361) {
  throw new Error("T49 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_buddhist_histories_sectarian_records_t49",
    label: "大正藏 T49 结集、法灭、部派论书与佛教史传固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T49 目录下、文件名卷号一致的 T49n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T49 的 14 份来源记录横跨结集传说、法住与法灭预言、部派论书、中国佛教通史、编年史及高丽史传杂录。DILA 为 14 份记录分配 14 个不同作品权威号；T2031–T2033 是关系密切但题名、责任、译者和全文不同的部派论书，T2034–T2038 是相互引用或续修但范围不同的史传，T2037/T2038 为本编与续集，T2039 的 TEI 撰者题记与 DILA 责任标签还需并列保留。相同主题、史料、作者传统、编年结构、引文或机器文本重叠均不能自动合并作品或表达，也不能把史传与论书标成佛陀逐字亲说。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T49n/, "T"),
      volume: "T49",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v3.6.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T49 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t49-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T49: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T49 Git tree 不一致`);
    }
  }
  console.log(`CBETA T49 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T49 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
