...
const version = "0.1.0";
const snapshotVersion = "1.3.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t25.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
...
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T25"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T25 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T25\/T25n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 15) throw new Error(`T25 固定来源记录应为 15，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 20558857) {
  throw new Error("T25 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_sutra_commentary_t25",
    label: "大正藏 T25 释经论部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T25 目录下、文件名卷号一致的 T25n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "释经论部含阿含论释、《大智度论》、金刚般若论释及般若要义根本颂与复注，也含同本异译、a/b 异本、内嵌根本文本和传统作者归属有争议的论书；来源文件数不是去重作品数，更不是佛陀亲说经文数或全球佛典分母。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T25n/, "T"),
      volume: "T25",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v1.2.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T25 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t25-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T25: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T25 Git tree 不一致`);
    }
  }
  console.log(`CBETA T25 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T25 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
