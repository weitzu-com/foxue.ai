...
const registryPath = resolve(root, "data/gbcr/registry-v3.9.0.json");
const sourceSnapshotsPath = resolve(root, "data/gbcr/source-snapshots-v0.9.0.json");
...
const cbetaCatalogPath = resolve(root, "data/corpus/cbeta/catalog-v2.8.0.json");
const cbetaManifestPath = resolve(root, "data/corpus/cbeta/manifest-v2.8.0.json");
const cbetaRegistryPath = resolve(root, "data/gbcr/registry-cbeta-v2.8.0.json");
...
requireValue(registry.registry?.version === "3.9.0", "登记册版本不匹配");
...
requireValue(sourceSnapshots.version === "0.9.0", "来源候选快照版本不匹配");
...
requireValue(chineseFamily?.candidateExpressionRecords === 1495, "汉译 T01–T21 候选记录未写入来源族");
requireValue(chineseFamily?.controlledExpressionRecords === 1495, "汉译 T01–T21 受控记录数不匹配");
requireValue(chineseFamily?.candidateExpressionBytes === 334710326, "汉译 T01–T21 候选字节数未写入来源族");
requireValue(chineseFamily?.controlledExpressionBytes === 334710326, "汉译 T01–T21 受控字节数不匹配");
...
requireValue(cbetaCatalog?.files?.length === 1481, "CBETA 受控目录来源记录数漂移");
requireValue(cbetaManifest?.files?.length === 1481, "CBETA 资产清单来源记录数漂移");
requireValue(cbetaRegistry?.works?.length === 1296, "CBETA 书目实体数漂移");
...
const controlledFiles = [
  ["registry-v3.9.0.json", raw],
  ["source-snapshots-v0.9.0.json", sourceSnapshotsRaw],
  ...
  ["catalog-v2.8.0.json", cbetaCatalogRaw],
  ["manifest-v2.8.0.json", cbetaManifestRaw],
  ["registry-cbeta-v2.8.0.json", cbetaRegistryRaw],
...
requireValue(registry.registry.version === "3.9.0", "当前 GBCR 版本必须为 v3.9.0");
requireValue(registry.works.length === 1585, "v3.9 必须登记 1,585 个可追踪作品实体");
requireValue(expressions.length === 1770, "v3.9 必须登记 1,770 个文本表达或见证");
requireValue(expressions.filter((expression) => expression.fullSourceText).length === 1737, "v3.9 必须登记 1,737 个完整文本表达或见证");
requireValue(segmentCount === 2135243, "v3.9 稳定行段总数漂移");
...
requireValue(provisionalCbetaWorks.length === 1154, "T01–T21 新增经号必须保留 1,154 个暂定书目实体");
requireValue(
  provisionalCbetaWorks.every((work) => work.relationDecision?.includes("不据此声称已经完成作品级去重")),
  "T01–T21 暂定书目实体缺少作品去重边界",
);
