...
const registryPath = resolve(root, "data/gbcr/registry-v5.6.0.json");
const sourceSnapshotsPath = resolve(root, "data/gbcr/source-snapshots-v2.6.0.json");
...
const checksumPath = resolve(root, "data/gbcr/checksums-v5.6.0.sha256");
...
const cbetaCatalogPath = resolve(root, "data/corpus/cbeta/catalog-v4.5.0.json");
const cbetaManifestPath = resolve(root, "data/corpus/cbeta/manifest-v4.5.0.json");
const cbetaRegistryPath = resolve(root, "data/gbcr/registry-cbeta-v4.5.0.json");
...
requireValue(registry.registry?.version === "5.6.0", "登记册版本不匹配");
...
requireValue(sourceSnapshots.version === "2.6.0", "来源候选快照版本不匹配");
...
requireValue(chineseFamily?.candidateExpressionRecords === 1862, "汉译 T01–T38 候选记录未写入来源族");
requireValue(chineseFamily?.controlledExpressionRecords === 1862, "汉译 T01–T38 受控记录数不匹配");
requireValue(chineseFamily?.candidateExpressionBytes === 577908730, "汉译 T01–T38 候选字节数未写入来源族");
requireValue(chineseFamily?.controlledExpressionBytes === 577908730, "汉译 T01–T38 受控字节数不匹配");
...
requireValue(cbetaCatalog?.files?.length === 1848, "CBETA 受控目录来源记录数漂移");
requireValue(cbetaManifest?.files?.length === 1848, "CBETA 资产清单来源记录数漂移");
requireValue(cbetaRegistry?.works?.length === 1630, "CBETA 书目实体数漂移");
requireValue(cbetaRegistry?.works?.flatMap((work) => work.expressions)?.length === 1848, "CBETA 文本表达数漂移");
...
const controlledFiles = [
  ["registry-v5.6.0.json", raw],
  ["source-snapshots-v2.6.0.json", sourceSnapshotsRaw],
  ...
  ["catalog-v4.5.0.json", cbetaCatalogRaw],
  ["manifest-v4.5.0.json", cbetaManifestRaw],
  ["registry-cbeta-v4.5.0.json", cbetaRegistryRaw],
...
requireValue(registry.registry.version === "5.6.0", "当前 GBCR 版本必须为 v5.6.0");
requireValue(registry.works.length === 1919, "v5.6 必须登记 1,919 个可追踪作品实体");
requireValue(expressions.length === 2137, "v5.6 必须登记 2,137 个文本表达或见证");
requireValue(expressions.filter((expression) => expression.fullSourceText).length === 2094, "v5.6 必须登记 2,094 个完整文本表达或见证");
requireValue(segmentCount === 3585922, "v5.6 稳定行段总数漂移");
