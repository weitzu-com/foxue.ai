...
const version = "0.1.0";
const snapshotVersion = "2.2.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "797dffe3e6ff6e1e4c6d53be64c4a12f0373e354";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t34.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T34"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T34 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T34"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T34 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T34\/T34n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 13) throw new Error(`T34 固定来源记录应为 13，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 13196188) {
  throw new Error("T34 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_sutra_commentary_t34",
    label: "大正藏 T34 经疏部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T34 目录下、文件名卷号一致的 T34n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T34 的 13 份来源记录是《法华经》《观世音菩萨普门品》与《金刚三昧经》的文句、玄论、义疏、游意、玄赞、宗要、论及再注释；根本经、章节、直接注释、教义阐释、讲说记录和再注释必须分层计量，不能把经疏目录位置误作佛陀逐字亲说归属。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T34n/, "T"),
      volume: "T34",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v2.1.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T34 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t34-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T34: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T34 Git tree 不一致`);
    }
  }
  console.log(`CBETA T34 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T34 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
