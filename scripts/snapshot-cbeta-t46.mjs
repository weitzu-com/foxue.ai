...
const version = "0.1.0";
const snapshotVersion = "3.4.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "519b7832c516e422e674816054f570e894f70e38";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t46.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T46"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T46 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T46"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T46 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T46\/T46n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 46) throw new Error(`T46 固定来源记录应为 46，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 12140911) {
  throw new Error("T46 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_tiantai_meditation_rituals_t46",
    label: "大正藏 T46 天台止观、教观、忏仪与显密汇编固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T46 目录下、文件名卷号一致的 T46n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T46 的 46 份来源记录横跨天台止观根本著作与注疏、同作者异作、教观纲要、宗派史料、忏仪、礼赞以及显密汇编。DILA 为 46 份记录分配 46 个不同作品权威号；根本论—注疏、仪轨—补助仪、相似忏法、集撰与未署名题记必须分层。共同宗派、作者、尊格、功能、引文或高文本重叠均不能自动合并作品或表达，也不能把东亚撰述标成佛陀逐字亲说。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T46n/, "T"),
      volume: "T46",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v3.3.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T46 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t46-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T46: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T46 Git tree 不一致`);
    }
  }
  console.log(`CBETA T46 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T46 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
