...
const registryPath = "data/gbcr/registry-v6.19.0.json";
...
const outputPath = "data/gbcr/buddha-word-scope-audit-v1.4.0.json";
const [registryBytes, dergeInventoryBytes, catalogBytes] = await Promise.all([
  readFile(resolve(root, registryPath)),
  readFile(resolve(root, dergeInventoryPath)),
  readFile(resolve(root, catalogPath)),
]);
const registry = JSON.parse(registryBytes.toString("utf8"));
const dergeInventory = JSON.parse(dergeInventoryBytes.toString("utf8"));
const beyondTaishoCatalog = JSON.parse(catalogBytes.toString("utf8"));
const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const jsonRaw = (value) => `${JSON.stringify(value, null, 2)}\n`;

if (registry.registry.version !== "6.19.0" || registry.works.length !== 3394) {
  throw new Error("GBCR v6.19 基线漂移");
}
if (dergeInventory.version !== "0.3.0" || dergeInventory.totals.topLevelCatalogRecords !== 1122) {
  throw new Error("德格甘珠尔目录基线漂移");
}
if (beyondTaishoCatalog.version !== "1.0.0" || beyondTaishoCatalog.files.length !== 3) {
  throw new Error("大正藏以外佛說經目錄基線漂移");
}
...
function classifyNanchuan(work) {
  const ids = work.externalIds.cbeta ?? [];
  const numbers = [...new Set(ids.map((id) => {
    const fileMatch = /^N\d+n(\d{4})/i.exec(id);
    const workMatch = /^N(\d{4})$/i.exec(id);
    return Number((fileMatch ?? workMatch)?.[1]);
  }).filter((value) => Number.isInteger(value)))];
  if (numbers.length !== 1) throw new Error(`${work.id} 的南傳經號無法唯一分類：${ids.join(",")}`);
  const number = numbers[0];
  if (number >= 4 && number <= 12) {
    return { category: "traditional_sutra_or_discourse_canon_member", ruleId: `cbeta-nanchuan-sutta-${String(number).padStart(4, "0")}` };
  }
  if ([13, 14, 15, 16, 17, 18, 20, 21].includes(number)) {
    return { category: "mixed_scriptural_collection_scope_boundary", ruleId: `cbeta-nanchuan-mixed-khuddaka-${String(number).padStart(4, "0")}` };
  }
  throw new Error(`${work.id} 的南傳經號不在本批次經藏範圍：N${String(number).padStart(4, "0")}`);
}

function classifyFangshan(work) {
  const ids = work.externalIds.cbeta ?? [];
  const numbers = [...new Set(ids.map((id) => /^F\d+n(\d{4})/i.exec(id)?.[1]).filter(Boolean))];
  if (numbers.length !== 1) throw new Error(`${work.id} 的房山經號無法唯一分類：${ids.join(",")}`);
  return {
    category: "traditional_sutra_or_discourse_canon_member",
    ruleId: `cbeta-fangshan-translated-sutra-${numbers[0]}`,
  };
}
...
function classifyWork(work) {
  ...
  if (work.id.startsWith("gbcr:work:nanchuan-")) return { sources, result: classifyNanchuan(work) };
  if (work.id.startsWith("gbcr:work:fangshan-")) return { sources, result: classifyFangshan(work) };
  if (sources.includes("cbeta_xml_p5")) return { sources, result: classifyCbeta(work) };
  if (sources.includes("esukhia_derge_kangyur")) return { sources, result: classifyDerge(work) };
  if (sources.includes("suttacentral_bilara")) return { sources, result: classifySuttaCentral(work) };
  throw new Error(`${work.id} 没有范围审计规则支持的来源`);
}

const registryIds = new Set(registry.works.map((work) => work.id));
const fangshanWorks = [...beyondTaishoCatalog.files.reduce((works, file) => {
  if (registryIds.has(file.workId)) return works;
  const existing = works.get(file.workId) ?? {
    id: file.workId,
    canonicalTitleZh: file.workTitle,
    externalIds: { cbeta: [] },
    expressions: [],
  };
  existing.externalIds.cbeta.push(file.id);
  existing.expressions.push({
    sourceSnapshotId: "cbeta_xml_p5",
    fullSourceText: file.completeness === "complete_source_file",
  });
  works.set(file.workId, existing);
  return works;
}, new Map()).values()];
if (fangshanWorks.length !== 2) throw new Error(`房山新增佛說經作品應為 2，實際 ${fangshanWorks.length}`);

const classified = [...registry.works, ...fangshanWorks].map((work) => {
  const { sources, result } = classifyWork(work);
...
const strictCandidateWorks = classified.filter((work) => work.strictSutraScope.startsWith("included_candidate"));
const summary = {
  registeredWorksAudited: classified.length,
  registeredWorksUnclassified: 0,
  ruleClassifiedWorks: classified.length,
  ...
  categoryCounts: countBy(classified, "category"),
  strictScopeDecisionCounts: countBy(classified, "strictSutraScope"),
  globalDenominatorImpact: "none_until_scope_policy_identity_deduplication_and_independent_review",
};
if (summary.registeredWorksAudited !== 3396) throw new Error("作品范围审计没有覆盖 v6.19 的 3,394 部加上 2 部房山漢譯佛說經");
if (new Set(classified.map((work) => work.workId)).size !== classified.length) throw new Error("作品范围审计出现重复作品");
...
if (summary.strictSutraCandidateWorks !== 1304 || summary.strictSutraCandidateWorksWithFullSource !== 1303) {
  throw new Error(`严格经藏候选统计漂移：${summary.strictSutraCandidateWorks}/${summary.strictSutraCandidateWorksWithFullSource}`);
}
...
const audit = {
  ...
  inputs: {
    ...
    beyondTaishoSutraCatalog: { path: catalogPath, sha256: sha256(catalogBytes) },
  },
  policy: {
    targetTerm: "佛陀教说／佛经",
...
if (verifyMode) {
  ...
  console.log(`佛陀教说范围审计 v1.4 可复现：${classified.length} 部登记作品全部分类，${summary.strictSutraCandidateWorks} 部进入严格经藏候选；全球分母仍不可发布。`);
} else {
  await writeFile(resolve(root, outputPath), raw);
  console.log(`佛陀教说范围审计 v1.4 已生成：${classified.length} 部登记作品全部分类，0 部遗漏，0 部独立专家批准。`);
}
