...
const nanchuanCatalogPath = "data/corpus/cbeta/nanchuan-catalog-v1.0.0.json";
const outputPath = "data/gbcr/buddha-word-scope-audit-v1.3.0.json";
const [registryBytes, dergeInventoryBytes, nanchuanCatalogBytes] = await Promise.all([
  readFile(resolve(root, registryPath)),
  readFile(resolve(root, dergeInventoryPath)),
  readFile(resolve(root, nanchuanCatalogPath)),
]);
const registry = JSON.parse(registryBytes.toString("utf8"));
const dergeInventory = JSON.parse(dergeInventoryBytes.toString("utf8"));
const nanchuanCatalog = JSON.parse(nanchuanCatalogBytes.toString("utf8"));
...
if (nanchuanCatalog.version !== "1.0.0" || nanchuanCatalog.files.length !== 45) {
  throw new Error("南傳經藏目錄基線漂移");
}
...
const nanchuanWorks = [...nanchuanCatalog.files.reduce((works, file) => {
  ...
  if (file.authorityIds?.nanchuanNumber && !existing.externalIds.cbeta.includes(file.authorityIds.nanchuanNumber)) {
    existing.externalIds.cbeta.push(file.authorityIds.nanchuanNumber);
  }
...
if (nanchuanWorks.length !== 17) throw new Error(`南傳經藏作品應為 17，實際 ${nanchuanWorks.length}`);

const classified = [...registry.works, ...nanchuanWorks].map((work) => {
...
if (summary.registeredWorksAudited !== 3394) throw new Error("作品范围审计没有覆盖 v6.16 的 3,377 部加上 17 部南傳漢譯作品");
...
const expectedCategoryCounts = {
  ...
  traditional_sutra_or_discourse_canon_member: 968,
};
...
if (summary.strictSutraCandidateWorks !== 1302 || summary.strictSutraCandidateWorksWithFullSource !== 1301) {
...
const audit = {
  ...
  inputs: {
    ...
    nanchuanCatalog: { path: nanchuanCatalogPath, sha256: sha256(nanchuanCatalogBytes) },
  },
  policy: {
    targetTerm: "佛陀教说／佛经",
...
if (verifyMode) {
  ...
  console.log(`佛陀教说范围审计 v1.3 可复现：${classified.length} 部登记作品全部分类，${summary.strictSutraCandidateWorks} 部进入严格经藏候选；全球分母仍不可发布。`);
} else {
  await writeFile(resolve(root, outputPath), raw);
  console.log(`佛陀教说范围审计 v1.3 已生成：${classified.length} 部登记作品全部分类，0 部遗漏，0 部独立专家批准。`);
}
