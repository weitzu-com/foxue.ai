...
const version = "0.1.0";
const snapshotVersion = "2.6.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "7a8a274c9658ca4d8a1ee7d5e18bac9187764c7f";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t38.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T38"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T38 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T38"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T38 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T38\/T38n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 18) throw new Error(`T38 固定来源记录应为 18，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 13495590) {
  throw new Error("T38 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_nirvana_medicine_buddha_maitreya_vimalakirti_commentary_t38",
    label: "大正藏 T38 涅槃、药师、弥勒与维摩经疏部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T38 目录下、文件名卷号一致的 T38n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T38 的 18 份来源记录是《大般涅槃经》《药师经》、弥勒诸经及《维摩经》的东亚玄义、游意、宗要、义记、经疏、略疏与再注释；共同根经、同一作者、同一教说传统、略本关系和互相关联目录记录必须分层计量，不能把经疏目录位置或引文误作佛陀逐字亲说归属。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T38n/, "T"),
      volume: "T38",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v2.5.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T38 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t38-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T38: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T38 Git tree 不一致`);
    }
  }
  console.log(`CBETA T38 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T38 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
