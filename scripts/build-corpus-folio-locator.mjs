...
const loadersPath = resolve(root, "src/lib/corpus-folio-locator-loaders.generated.ts");
...
function buildLoadersSource(shardCount) {
  const cases = Array.from({ length: shardCount }, (_, id) => {
    return `    case ${id}:\n      return ((await import("@/data/corpus-folio-locator-chunks/${id}.json")).default).works;`;
  }).join("\n");

  return `// Generated by scripts/build-corpus-folio-locator.mjs. Do not edit.
...
if (write) {
  ...
  if (actualLoaders !== expectedLoaders) {
    throw new Error("src/lib/corpus-folio-locator-loaders.generated.ts 与定位分片不一致；运行 pnpm build:corpus-folio-locator");
  }
