...
const version = "0.1.0";
const snapshotVersion = "2.9.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const expectedTree = "1cca22b31b60b7811cf112f1ec7fe22664c39a7f";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t41.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
const sourceRoot = resolve(sourceArgument.slice("--source-dir=".length));
const actualCommit = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualCommit !== expectedCommit) throw new Error(`上游工作树必须固定到 ${expectedCommit}`);
const actualTree = execFileSync("git", ["-C", sourceRoot, "rev-parse", "HEAD:T/T41"], { encoding: "utf8" }).trim();
if (actualTree !== expectedTree) throw new Error(`T41 Git tree 必须固定到 ${expectedTree}`);

const sha256 = (value) => createHash("sha256").update(value).digest("hex");
const digestPaths = (paths) => sha256([...paths].sort().join("\n"));
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T41"],
  { maxBuffer: 8 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T41 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T41\/T41n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 3) throw new Error(`T41 固定来源记录应为 3，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 16077486) {
  throw new Error("T41 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_abhidharma_kosa_commentary_t41",
    label: "大正藏 T41 俱舍论注疏部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T41 目录下、文件名卷号一致的 T41n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "T41 的 3 份来源记录是普光《俱舍论记》、法宝《俱舍论疏》和圆晖《俱舍论颂疏》三部唐代阿毘达磨注疏；前两部直接解释玄奘译《阿毘达磨俱舍论》T1558，后一部兼依 T1558 与世亲《俱舍论本颂》T1560。共同根本论书、三十卷规模、题名或正文引文不能把不同作者、不同注释结构和不同解释范围误作同一作品、论本表达或佛陀逐字亲说归属。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T41n/, "T"),
      volume: "T41",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v2.8.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T41 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t41-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T41: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T41 Git tree 不一致`);
    }
  }
  console.log(`CBETA T41 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T41 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
