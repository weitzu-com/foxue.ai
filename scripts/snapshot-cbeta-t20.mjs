...
const version = "0.1.0";
const snapshotVersion = "0.8.0";
const expectedCommit = "2b8ab8d5e4fe957a9b94f2cde01cb0d2e2dcd2b9";
const sourceArgument = process.argv.find((argument) => argument.startsWith("--source-dir="));
if (!sourceArgument) {
  console.error("用法：node scripts/snapshot-cbeta-t20.mjs --source-dir=/固定提交的/xml-p5 [--write|--verify]");
  process.exit(1);
}
...
const treeRaw = execFileSync(
  "git",
  ["-C", sourceRoot, "ls-tree", "-r", "-z", "--long", "HEAD", "T/T20"],
  { maxBuffer: 4 * 1024 * 1024 },
).toString("utf8");
const treeEntries = treeRaw.split("\0").filter(Boolean).map((entry) => {
  const match = entry.match(/^\d+\s+blob\s+([a-f0-9]{40})\s+(\d+)\t(.+)$/);
  if (!match) throw new Error(`无法解析 T20 Git tree 记录：${entry}`);
  return { sha: match[1], size: Number(match[2]), path: match[3] };
}).filter((entry) => /^T\/T20\/T20n[0-9A-Za-z_]+\.xml$/.test(entry.path));

if (treeEntries.length !== 184) throw new Error(`T20 固定来源记录应为 184，实际 ${treeEntries.length}`);
if (treeEntries.reduce((sum, entry) => sum + entry.size, 0) !== 24220376) {
  throw new Error("T20 固定来源字节数漂移");
}
...
const inventory = {
  ...
  subset: {
    id: "taisho_esoteric_t20",
    label: "大正藏 T20 密教部固定来源记录",
    inclusionRule: "固定 Git tree 中 T/T20 目录下、文件名卷号一致的 T20n*.xml",
    recordUnit: "TEI P5 source record",
    denominatorCaveat: "密教部含译经、陀罗尼、仪轨、念诵法、赞颂、注校、请来或口受材料、失译、局部材料与版本见证；来源文件数不是佛陀亲说作品数，也不是全球佛典分母。",
  },
  totals: {
  ...
  records: treeEntries.map((entry) => {
    ...
    return {
      sourceRecordId,
      canonWitnessId: sourceRecordId.replace(/^T20n/, "T"),
      volume: "T20",
...
const baseSnapshotPath = "data/gbcr/source-snapshots-v0.7.0.json";
...
if (cbetaSource.candidateSubsets.some((subset) => subset.id === inventory.subset.id)) {
  throw new Error("既有来源快照已经包含 T20 子集");
}

const inventoryPath = `data/gbcr/cbeta-taisho-t20-inventory-v${version}.json`;
    ...
    ? {
        ...
        candidateSubsets: [
          ...source.candidateSubsets,
          {
            ...
            groups: { T20: inventory.totals.records },
...
if (process.argv.includes("--verify")) {
  for (const [relativePath, expected] of [[inventoryPath, inventoryRaw], [snapshotPath, snapshotRaw]]) {
    if (await readFile(resolve(root, relativePath), "utf8") !== expected) {
      throw new Error(`${relativePath} 与固定 T20 Git tree 不一致`);
    }
  }
  console.log(`CBETA T20 来源快照可复现：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
  ...
  console.log(`CBETA T20 来源快照已生成：${inventory.totals.records} 份、${inventory.totals.upstreamBytes} 字节。`);
} else {
