...
1. 核对 `data/gbcr/checksums-v0.8.0.sha256`。
...
4. 运行 `pnpm verify:cbeta-catalog`、`pnpm verify:suttacentral-catalog`、`pnpm verify:suttacentral-dn-catalog`、`pnpm verify:corpus-catalog` 与 `pnpm verify:cbeta-pilot`，核对受控批次、60 个完整文本表达、39 个 CBETA TEI 来源资产和 60 个 SuttaCentral JSON 来源资产的哈希、权利声明、结构和稳定锚点。
...
平台无关的最低恢复方式：在任意支持 Node 22 的环境执行 `pnpm build` 和 `pnpm start`。当前生产平台是 Vercel，但代码不依赖专有运行时才能阅读 60 个受控完整文本表达与覆盖登记册。
...
4. 部署后验证 `/api/health`、`/api/v1/corpus/coverage`、`/fugai`、汉文代表页、巴利《法句经》首尾锚点与《长部》DN1/DN34 代表锚点；
