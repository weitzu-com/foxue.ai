...
截至 2026-08-11，方案已经完成第六个可审计语料里程碑：

- 21 部去重作品、25 个 CBETA 完整文本表达进入受控仓库，共 21,845 个版页、603,032 个稳定行段；
...
- 语料发布包扩展为 21,911 个对象，并保留边缘读取失败时的本地完整回退；
- 新增受控批次定义、确定性目录生成、批量导入、作品级归并与版本保留机制，为后续按批扩展 881 条候选记录提供统一入口。

下一语料门是按同一目录学纪律继续扩展剩余大部经典，并建立逐批人工抽检与机构级离线保存；任何阶段都不得把物理文件数冒充译本数或作品数。
