...
截至 2026-08-11，方案已经完成第二个可审计语料里程碑：

- 9 部 CBETA 完整 TEI 原文进入受控仓库，共 596 个版页、16,285 个稳定行段；
- 每份原文同时记录固定上游提交、Git blob SHA-1、上游与本地 SHA-256、授权头部、字节规范化、卷次、版页和关键锚点；
- 冻结大正藏 T01–T17 的 881 条汉译经藏候选文本记录，当前 8 条进入受控全文库，阶段记录进度为 0.91%；
- 0.91% 只表示固定来源中的文本记录进度，不代表去重作品覆盖率，也不代表全球佛典覆盖率；
- 语料发布包扩展为 616 个不可变对象，并保留边缘读取失败时的本地完整回退；
- 新增批量导入、目录生成、可复现校验与版本保留机制，为后续按批扩展 881 条候选记录提供统一入口。
