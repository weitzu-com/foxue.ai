...
截至 2026-08-11，方案已经完成第三个可审计语料里程碑：

- 12 部去重作品、14 个 CBETA 完整 TEI 文本进入受控仓库，共 1,085 个版页、29,905 个稳定行段；
- 每份原文同时记录固定上游提交、Git blob SHA-1、上游与本地 SHA-256、授权头部、字节规范化、卷次、版页和关键锚点；
- 冻结大正藏 T01–T17 的 881 条汉译经藏候选文本记录，当前 13 条进入受控全文库，阶段记录进度为 1.48%；
- GBCR v0.2.1 进一步逐条冻结这 881 条记录的路径、Git 对象哈希与字节数；合计原始体量 247,280,257 字节，使导入范围和剩余工作可逐项复算；
- 13 条受控候选记录共 5,149,387 个上游字节，占固定候选字节数 2.08%；记录与字节进度都不代表去重作品覆盖率或全球佛典覆盖率；
- 《楞伽经》T0670、T0671、T0672 被建模为一个规范作品的三个汉译文本表达，避免把异译文件重复计算为三部作品；
- 语料发布包扩展为 1,115 个对象，并保留边缘读取失败时的本地完整回退；
- 新增受控批次定义、确定性目录生成、批量导入、作品级归并与版本保留机制，为后续按批扩展 881 条候选记录提供统一入口。

下一语料门是优先导入阿含、般若、华严、宝积与涅槃等大部经典，同时建立异译、别本、章节与作品级关系，避免把文件数误当作品数。
