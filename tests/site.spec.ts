...
test("站点地图按 Hub、经目和版页模板分层", async ({ request }) => {
  ...
  expect(hubs).toContain("<loc>https://www.foxue.ai/xue/dujing</loc>");
  expect(hubs).toContain("<loc>https://www.foxue.ai/xue/xuanjing</loc>");
...
test("关键 SEO 页面输出自指 canonical、og:url 与 twitter card", async ({ request }) => {
  const cases = [
    ...
    ["/duidu/ebt", "https://www.foxue.ai/duidu/ebt"],
    ...
    ["/xue/dujing", "https://www.foxue.ai/xue/dujing"],
    ["/xue/xuanjing", "https://www.foxue.ai/xue/xuanjing"],
...
test("关键 SEO 页面输出页面级 JSON-LD", async ({ request }) => {
  const cases = [
    ...
    },
    {
      path: "/xue/dujing",
      required: [
        ["https://www.foxue.ai/xue/dujing#page", "WebPage"],
        ["https://www.foxue.ai/xue/dujing#breadcrumb", "BreadcrumbList"],
        ["https://www.foxue.ai/xue/dujing#method", "LearningResource"],
        ["https://www.foxue.ai/xue/dujing#steps", "HowTo"],
        ["https://www.foxue.ai/xue/dujing#questions", "FAQPage"],
      ],
    },
    {
      path: "/xue/xuanjing",
      required: [
        ["https://www.foxue.ai/xue/xuanjing#page", "CollectionPage"],
        ["https://www.foxue.ai/xue/xuanjing#breadcrumb", "BreadcrumbList"],
        ["https://www.foxue.ai/xue/xuanjing#routes", "ItemList"],
        ["https://www.foxue.ai/xue/xuanjing#questions", "FAQPage"],
      ],
    },
    {
      path: "/xue/meiri",
    ...
    },
    {
      path: "/xue/faju",
      required: [
        ["https://www.foxue.ai/xue/faju#page", "CollectionPage"],
        ["https://www.foxue.ai/xue/faju#breadcrumb", "BreadcrumbList"],
        ["https://www.foxue.ai/xue/faju#source-dossier", "LearningResource"],
      ],
    },
    {
      path: "/xue/xinjing",
  ...
  for (const { path, required } of cases) {
    ...
    if (path === "/duidu") {
      ...
      expect(dossiers?.numberOfItems).toBe(4);
      expect(dossiers?.itemListElement).toHaveLength(4);
    }

    if (path === "/duidu/ebt") {
      const evidenceCases = items.find(
...
test("对读书案让三类读者进入四份受控卷宗且守住比较边界", async ({ page }) => {
  await page.goto("/duidu");

  await expect(page.getByRole("heading", { level: 1, name: /并读.*不是把差异.*磨成同一句/ }))
    .toBeVisible();
  await expect(page.locator("[data-comparison-dossier]")).toHaveCount(4);
  ...
  await expect(page.getByRole("link", { name: /打开三份 EBT 书案/ })).toHaveAttribute("href", "/duidu/ebt");
...
test("汉巴 EBT 书案开放原文、反证与零裁决状态", async ({ page }) => {
  await page.goto("/duidu/ebt");

  await expect(page.getByRole("heading", { level: 1, name: /相似.*先不是相同/ })).toBeVisible();
  await expect(page.locator("[data-ebt-case]")).toHaveCount(3);
  await expect(page.getByText("5,161", { exact: true })).toBeVisible();
  await expect(page.getByText("P0 证据包", { exact: true })).toBeVisible();
  await expect(page.getByText("已裁决", { exact: true })).toBeVisible();
  await expect(page.getByText("0 / 2 真人复核", { exact: true })).toHaveCount(3);
  await expect(page.getByText(/EA 50\.8 开头与 MN 21/)).toBeVisible();
  await expect(page.getByText(/后段的邪见、纠正与蛇喻却更接近 MN 22/)).toBeVisible();
  await expect(page.getByRole("link", { name: /从首行读汉译/ }).first())
    .toHaveAttribute("href", "/jingzang/zaahanjing#T0099.001.0001a06");
  await expect(page.locator('link[rel="canonical"]'))
    .toHaveAttribute("href", "https://www.foxue.ai/duidu/ebt");
...
test("首页核心任务可见且没有水平溢出", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /找到第一条路径/ }))
...
test("法华经七关以稳定原典、同品见证和本地进度打开二十八品", async ({ page, request }) => {
  await page.goto("/xue/fahuajing");

  await expect(page.locator("h1")).toContainText("《法华经》很长");
  await expect(page.getByRole("heading", { name: "七处转折，打开二十八品。" })).toBeVisible();
  await expect(page.locator(".learning-overview__grid > li")).toHaveCount(7);
  await expect(page.getByText("当前阅读底本：姚秦·鸠摩罗什译 T0262")).toBeVisible();
  await expect(page.getByText(/只链接同品，不宣称逐句或逐词对应/)).toBeVisible();
  await expect(page.getByText("编辑路标不是经文")).toBeVisible();
  await expect(page.getByRole("button", { name: "复制引文与出处" })).toBeVisible();
  await expect(page.getByRole("button", { name: /写研读笺/ })).toBeVisible();
  await expect(page.locator('a[href="/jingzang/fahuajing/001-0007a#T0262.001.0007a21"]'))
    .toContainText("T0262.001.0007a21–28");
  await expect(page.locator('a[href^="/jingzang/gutenberg-en-lotus-soothill/001-c02#"]'))
    .toBeVisible();

  await page.getByRole("button", { name: /第 6 关.*未标记/ }).click();
  await expect(page).toHaveURL(/#day-6$/);
  await expect(page.getByRole("heading", { level: 2, name: "让时间尺度忽然打开" })).toBeVisible();
  await expect(page.locator('a[href="/jingzang/fahuajing/005-0042b#T0262.005.0042b10"]'))
    .toContainText("T0262.005.0042b10–13");
  await expect(page.locator('a[href^="/jingzang/gutenberg-en-lotus-soothill/001-c16#"]'))
    .toBeVisible();

  await page.getByRole("button", { name: /读完这一关/ }).click();
  expect(await page.evaluate(() =>
    window.localStorage.getItem("foxue:fahuajing-seven-gate-progress:v1"),
  )).toContain('"6":"completed"');
  await expect.poll(() => page.evaluate(() =>
    window.localStorage.getItem("foxue:study-path-activity:v1") ?? "",
  )).toContain('"id":"fahuajing","activeDay":7');

  await page.goto("/xue");
  const resumeCard = page.locator('[data-study-path-card="fahuajing"]');
  await expect(resumeCard).toBeVisible();
  await expect(resumeCard.getByText("下一步：第 7 关 · 已标记 1 / 7")).toBeVisible();
  await expect(resumeCard.getByRole("link", { name: /继续第 7 关/ }))
    .toHaveAttribute("href", "/xue/fahuajing#day-7");

  const primarySource = await request.get("/jingzang/fahuajing/005-0042b");
  expect(primarySource.ok()).toBeTruthy();
  expect(await primarySource.text()).toContain('id="T0262.005.0042b10"');
  const englishWitness = await request.get("/jingzang/gutenberg-en-lotus-soothill/001-c16");
  expect(englishWitness.ok()).toBeTruthy();
  expect(await englishWitness.text()).toContain(
    'id="GUTENBERG-LOTUS-SOOTHILL-1930.001.s0000033300"',
  );

  await page.goto("/xue/fahuajing#day-7");
  await page.setViewportSize({ width: 390, height: 844 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(390);
  const accessibility = await new AxeBuilder({ page })
    .withTags(["wcag2a", "wcag2aa", "wcag21aa", "wcag22aa"])
...
test("研读中心按静读、理解与校勘组织入口", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /不知道先读哪部/ }))
    .toHaveAttribute("href", "/xue/xuanjing");
...
test("选经页按目的、质地与时间连接六条可核验路径", async ({ page }) => {
  await page.goto("/xue/xuanjing");

  await expect(page.getByRole("heading", { level: 1, name: /先别问哪部最好.*先问此刻为何而读/ })).toBeVisible();
  await expect(page.getByText("选择是一张可修改的阅读假设，不是终身标签。")).toBeVisible();
  await expect(page.locator("[data-scripture-route]")).toHaveCount(6);
  await expect(page.locator('[data-scripture-route="daily"]')).toContainText("每次 3–10 分钟");
  await expect(page.locator('[data-scripture-route="heart"]')).toContainText("短不等于浅");
  await expect(page.locator('[data-scripture-route="lotus"]')).toContainText("七关是进入长经的路标");
  await expect(page.getByRole("link", { name: /打开三十段原典/ })).toHaveAttribute("href", "/xue/meiri");
  await expect(page.getByRole("link", { name: /打开三源《法句》/ })).toHaveAttribute("href", "/xue/faju");
  await expect(page.getByRole("link", { name: /开始《心经》第一天/ })).toHaveAttribute("href", "/xue/xinjing");
  await expect(page.getByRole("link", { name: /开始《金刚经》核读/ })).toHaveAttribute("href", "/xue/jingangjing");
  await expect(page.getByRole("link", { name: /开始《阿弥陀经》净读/ })).toHaveAttribute("href", "/xue/amituojing");
  await expect(page.getByRole("link", { name: /打开《法华经》七关/ })).toHaveAttribute("href", "/xue/fahuajing");
  await expect(page.getByRole("link", { name: /已经选好.*学习怎样读/ })).toHaveAttribute("href", "/xue/dujing");
  await expect(page.getByRole("link", { name: /浏览十个概念入口/ })).toHaveAttribute("href", "/gainian");
  await expect(page.getByRole("link", { name: /进入异译与跨本对读/ })).toHaveAttribute("href", "/duidu");
  await expect(page.locator('[data-analytics-event="scripture_path_selected"]')).toHaveCount(6);
...
test("读经方法页把原文、上下文、名相与复核串成可操作四步", async ({ page }) => {
  await page.goto("/xue/dujing");

  await expect(page.getByRole("heading", { level: 1, name: /读不懂.*不要急着找结论/ })).toBeVisible();
  await expect(page.getByText("佛教徒 · 读诵与受持", { exact: true })).toBeVisible();
  await expect(page.getByText("佛学爱好者 · 理解与探索", { exact: true })).toBeVisible();
  await expect(page.getByText("研究者 · 校读与论证", { exact: true })).toBeVisible();
  await expect(page.locator("[data-reading-step]")).toHaveCount(4);
  await expect(page.locator('[data-reading-step="identity"]')).toContainText("先认文本");
  await expect(page.locator('[data-reading-step="context"]')).toContainText("再读前后");
  await expect(page.locator('[data-reading-step="terms"]')).toContainText("圈出名相");
  await expect(page.locator('[data-reading-step="verify"]')).toContainText("最后复核");
  await expect(page.getByText("諸惡莫作，諸善奉行，自淨其意，是諸佛教。", { exact: true })).toBeVisible();
  await expect(page.getByText(/常见“众善奉行”与底本“诸善奉行”不同/)).toBeVisible();
  await expect(page.getByRole("link", { name: /打开原句与稳定行段/ }))
    .toHaveAttribute("href", "/jingzang/fajujing/002-0567b#T0210.002.0567b01");
  await expect(page.getByRole("link", { name: /继续汉译、巴利与英译三源研读/ }))
    .toHaveAttribute("href", "/xue/faju");
...
test("法句三源档案保留稳定引文与比较边界", async ({ page, request }) => {
  ...
  await expect(page.locator('a[href="/jingzang/fajujing/001-0562a#T0210.001.0562a13"]'))
    .toContainText("T0210.001.0562a13–14");
  await expect(page.locator('a[href="/jingzang/dhammapada-pali/001-dhp1-20#dhp1:1"]'))
    .toContainText("dhp1:1–6");
  await expect(page.locator('a[href="/jingzang/wikisource-en-dhp-muller/001-c01#WIKISOURCE-DHP-MULLER-1881.001.s0000000100"]'))
    .toContainText("WIKISOURCE-DHP-MULLER-1881.001.s0000000100");
  await expect(page.getByText("相近，不等于相同。", { exact: true })).toBeVisible();
  await expect(page.getByText(/不发布未经人工校勘的逐词或逐偈对齐结论/)).toBeVisible();
  await expect(page.getByText(/尚未标注外部具名佛学审校/)).toBeVisible();
...
test("SN 45.8 定义正文与八支实际变格均可进入八正道概念页并带原文问经", async ({ page }) => {
  for (const segmentId of ["sn45.8:3.2", "sn45.8:8.2", "sn45.8:7.2", "sn45.8:10.6"]) {
    await page.goto("/jingzang/samyutta-nikaya-sn45/008-sn45-8-0001-0044");
...
test("中英八正道明确别名的选文入口与问经答案保持一致", async ({ page }) => {
  const cases = [
    {
      href: "/jingzang/suttacentral-benshi-jing-t0765/127-t765-127-0001-0030",
      segmentId: "t765.127:1.11",
    },
    {
  ...
  for (const item of cases) {
    ...
    await page.evaluate((segmentId) => {
      ...
      if (!target) throw new Error(`Missing explicit eightfold path alias: ${segmentId}`);
      const range = document.createRange();
      range.selectNodeContents(target);
      const selection = window.getSelection();
      selection?.removeAllRanges();
...
test("经藏目录以服务端分页支持元数据检索与语种筛选", async ({ page, request }) => {
  ...
  await expect(page.getByText(/找到 259 个文本表达/)).toBeVisible();
...
test("覆盖登记册拒绝伪造全球百分比并公开可复算 API", async ({ page, request }) => {
  ...
  expect(coverage.candidateInventory.suttacentralSujatoEnglishKn).toMatchObject({
    controlledExpressions: 5,
    attachedExistingWorks: 5,
    attachedUids: ["iti", "snp", "ud", "kp", "cp"],
    newWorks: 0,
    sourceRecords: 309,
    stableSegments: 12524,
    sourceBytes: 824746,
    filesApprovedForReadingAndRetrieval: 309,
    filesApprovedForModelTraining: 0,
  });
  expect(coverage.candidateInventory.chineseSutraRecordSubset).toMatchObject({
    ...
    controlled: 2471,
    percentage: 100,
    sourceBytes: 855499447,
    controlledBytes: 855499447,
    bytePercentage: 100,
    ...
    t54InventorySha256: "fa445094b7bfe4f80c80deddb519fd12276f281bba54f87bc1d5814e437bbdd5",
    t55InventorySha256: "86a05a20d2884b3aaa77ab4b5f6061cdd9fa3a423bd91a2799a5c3a648a7f7d8",
    t85InventorySha256: "63ddc4f9d83530d741af5088045f20ec7588440b4f878a86fd83770662034523",
  ...
  expect(coverage.links).toMatchObject({
    ...
    suttacentralSujatoEnglishKnIngest: expect.stringContaining("suttacentral-sujato-en-kn-ingest-v1.0.0.json"),
    suttacentralSujatoEnglishKnRightsAudit: expect.stringContaining("suttacentral-sujato-en-kn-rights-audit-v1.0.0.json"),
    suttacentralSujatoEnglishKnBoundaryAudit: expect.stringContaining("sujato-en-kn-batch-v1.0.0.json"),
    ...
    chineseBuddhistCataloguesT55Inventory: expect.stringContaining("cbeta-taisho-t55-inventory-v0.1.0.json"),
    chineseBuddhistCataloguesT55BoundaryAudit: expect.stringContaining("batch-v4.22.0.json"),
    chineseLostAndSuspectedTextsT85Inventory: expect.stringContaining("cbeta-taisho-t85-inventory-v0.1.0.json"),
    chineseLostAndSuspectedTextsT85BoundaryAudit: expect.stringContaining("batch-v4.23.0.json"),
  });
  expect(coverage.candidateInventory.suttacentralPaliRootPilot).toMatchObject({
    denominator: 7288,
    controlled: 7288,
...
test("汉译 T55 佛教经录与书目完整受控且不误标为佛陀逐字亲说", async ({ page, request }) => {
  await page.goto("/jingzang/taisho-t2145/001-0001a");
  await expect(page.getByRole("heading", { level: 1, name: "出三藏記集" })).toBeVisible();
  await expect(page.getByText(/梁 · 僧祐撰/).first()).toBeVisible();

  await page.goto("/jingzang/taisho-t2168a/001-1087c");
  await expect(page.getByRole("heading", { level: 1, name: "惠運禪師將來教法目錄" })).toBeVisible();
  await expect(page.getByText(/日本 · 惠運撰/).first()).toBeVisible();

  await page.goto("/jingzang/taisho-t2174b/001-1111c");
  await expect(page.getByRole("heading", { level: 1, name: "禪林寺宗叡僧正目錄" })).toBeVisible();
  await expect(page.getByText(/传统责任题记未署名/).first()).toBeVisible();

  await page.goto("/jingzang/taisho-t2184/003-1178c");
  await expect(page.getByRole("heading", { level: 1, name: "新編諸宗教藏總錄" })).toBeVisible();
  await expect(page.getByText(/高麗 · 義天錄/).first()).toBeVisible();

  const representativePaths = [
    "/jingzang/taisho-t2145/001-0001a",
    "/jingzang/taisho-t2145/015-0114a",
    "/jingzang/taisho-t2154/001-0477a",
    "/jingzang/taisho-t2154/020-0723a",
    "/jingzang/taisho-t2168a/001-1087c",
    "/jingzang/taisho-t2168a/001-1089a",
    "/jingzang/taisho-t2168b/001-1089a",
    "/jingzang/taisho-t2168b/001-1092a",
    "/jingzang/taisho-t2174a/001-1108a",
    "/jingzang/taisho-t2174b/001-1111c",
    "/jingzang/taisho-t2184/001-1165b",
    "/jingzang/taisho-t2184/003-1178c",
  ];
  for (const path of representativePaths) expect((await request.get(path)).ok()).toBeTruthy();

  const coverage = await (await request.get("/api/v1/corpus/coverage")).json();
  expect(coverage.candidateInventory.chineseT55SourceRecords).toMatchObject({
    denominator: 42,
    controlled: 42,
    percentage: 100,
    fullSourceTexts: 42,
    partialSourceWitnesses: 0,
    verifiedEditionWitnesses: 0,
    relationAnnotatedRecords: 42,
    attributionBoundaryRecords: 42,
    unsignedResponsibilityRecords: 3,
    nonBuddhistReferenceRecords: 0,
    newWorks: 42,
    controlledWorks: 42,
    sameNumberBoundaryGroups: 2,
    scopeBoundaryGroups: 4,
    continuationBoundaryGroups: 4,
    sourceReuseBoundaryGroups: 3,
    sameAuthorCompanionWorkGroups: 2,
    relatedDistinctWorkGroups: 14,
  });

  const sitemap = await readSitemaps(request);
  for (const path of representativePaths) expect(sitemap).toContain(path);
});

test("汉译 T85 古逸与疑似部完整受控并保持异本、卷序及佛说边界", async ({ page, request }) => {
  await page.goto("/jingzang/taisho-t2732/001-0001a");
  await expect(page.getByRole("heading", { level: 1, name: "梁朝傅大士頌金剛經" })).toBeVisible();

  await page.goto("/jingzang/taisho-t2764a/001-0280a");
  await expect(page.getByRole("heading", { level: 1, name: "涅槃經義記" })).toBeVisible();

  await page.goto("/jingzang/taisho-t2764b/004-0294a");
  await expect(page.getByRole("heading", { level: 1, name: "大涅槃經義記卷第四" })).toBeVisible();

  await page.goto("/jingzang/taisho-t2865/001-1325a");
  await expect(page.getByRole("heading", { level: 1, name: "護身命經" })).toBeVisible();

  const representativePaths = [
    "/jingzang/taisho-t2732/001-0001a",
    "/jingzang/taisho-t2732/001-0008c",
    "/jingzang/taisho-t2764a/001-0280a",
    "/jingzang/taisho-t2764a/001-0294a",
    "/jingzang/taisho-t2764b/004-0294a",
    "/jingzang/taisho-t2764b/004-0304a",
    "/jingzang/taisho-t2830a/001-1268b",
    "/jingzang/taisho-t2830b/001-1269a",
    "/jingzang/taisho-t2865/001-1325a",
    "/jingzang/taisho-t2865/001-1326a",
    "/jingzang/taisho-t2917a/001-1462a",
    "/jingzang/taisho-t2917b/001-1462b",
    "/jingzang/taisho-t2920/001-1463b",
    "/jingzang/taisho-t2920/001-1464a",
  ];
  for (const path of representativePaths) expect((await request.get(path)).ok()).toBeTruthy();

  const coverage = await (await request.get("/api/v1/corpus/coverage")).json();
  expect(coverage.candidateInventory.chineseT85SourceRecords).toMatchObject({
    denominator: 192,
    controlled: 192,
    percentage: 100,
    fullSourceTexts: 192,
    partialSourceWitnesses: 0,
    provisionalRecords: 192,
    relationAnnotatedRecords: 192,
    attributionBoundaryRecords: 192,
    unsignedResponsibilityRecords: 160,
    buddhistCulturalReferenceRecords: 7,
    suspectedTextRecords: 57,
    newWorks: 192,
    controlledWorks: 192,
    sameNumberBoundaryGroups: 3,
    sameTitleBoundaryGroups: 9,
    irregularJuanSequenceGroups: 4,
    manuscriptCollationPendingGroups: 12,
    relatedDistinctWorkGroups: 22,
  });
...
test("Sujato CC0 小部英译挂接既有巴利作品并保留 Bilara 段落标识", async ({ page, request }) => {
  await page.goto("/jingzang/suttacentral-en-kp#kp1:1.1");
  await page.waitForURL(/\/jingzang\/suttacentral-en-kp\/001-kp1-0001-0012#kp1:1\.1$/);
  await expect(page.locator('[id="kp1:1.1"]')).toContainText("Homage to him, the blessed one");
  await expect(page.getByText("Bhikkhu Sujato").first()).toBeVisible();
  await expect(page.getByText("CC0").first()).toBeVisible();
  await expect(page.getByText(/英文译本/).first()).toBeVisible();

  await page.goto("/jingzang/suttacentral-en-iti#iti1:1.1");
  await page.waitForURL(/\/jingzang\/suttacentral-en-iti\/001-iti1-0001-0019#iti1:1\.1$/);
  await expect(page.locator('[id="iti1:1.1"]')).toContainText("This was said by the Buddha");

  const directory = await request.get("/jingzang/suttacentral-en-kp");
  const chapter = await request.get("/jingzang/suttacentral-en-kp/001-kp1-0001-0012");
  const iti = await request.get("/jingzang/suttacentral-en-iti/001-iti1-0001-0019");
  expect(directory.ok()).toBeTruthy();
  expect(chapter.ok()).toBeTruthy();
  expect(iti.ok()).toBeTruthy();
  const sitemap = await readSitemaps(request);
  expect(sitemap).toContain("/jingzang/suttacentral-en-kp/001-kp1-0001-0012");
  expect(sitemap).toContain("/jingzang/suttacentral-en-iti/001-iti1-0001-0019");
});
