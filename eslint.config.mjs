import { defineConfig, globalIgnores } from "eslint/config";
import nextVitals from "eslint-config-next/core-web-vitals";
import nextTs from "eslint-config-next/typescript";

export default defineConfig([
  ...nextVitals,
  ...nextTs,
  globalIgnores([
    ".next/**",
    ".vercel/**",
    "artifacts/**",
    "data/corpus/**/*.xml",
    "src/data/corpus-folio-index.generated.json",
    "src/data/corpus-sitemap-ledger.generated.json",
    "src/data/corpus-sitemap-chunks/**",
    "src/data/corpus-work-ledger.generated.json",
    "src/data/corpus-work-catalog-chunks/**",
    "src/lib/sitemap-chunk-loaders.generated.ts",
    "src/lib/corpus-work-catalog-loaders.generated.ts",
    "src/data/corpus-folio-locator-ledger.generated.json",
    "src/data/corpus-folio-locator-chunks/**",
    "src/lib/corpus-folio-locator-loaders.generated.ts",
    "infra/corpus-edge/worker-configuration.d.ts",
    "out/**",
    "build/**",
    "next-env.d.ts",
  ]),
]);
